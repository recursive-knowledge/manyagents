...
Lets ``oms.distill`` use the *user's* model without ``oms`` hosting inference;
``oms`` ships no keys. Resolution order (Settled): adapter ``distill_model()``
hook → configured ``OMS_LLM_*`` OpenAI-compatible fallback → hard error
(never a silent skip; asserted in ``oms.distill``).

Rate-limit detection: the general per-provider signal map is Open
(``oms.utils.md``). The two datasmith-proven schemas (Codex free-text reset,
Claude structured ``rate_limit_event``) are ported here verbatim because
``oms.utils.md`` Verification requires parsing canned Codex/Claude payloads;
...
def rate_limit_signal(raw_error: str, *, provider: str | None = None) -> RateLimit | None:
    ...
    The general per-provider map is Open (``oms.utils.md``).
    """
    if not raw_error:
        return None
...
class Provider(Protocol):
    """The model seam ``oms.distill`` curates through."""
...
class ProviderUnavailable(RuntimeError):
    ...
    configured ``OMS_LLM_*`` fallback (``oms`` ships no keys)."""


@dataclass
class OpenAICompatibleProvider:
    """Minimal OpenAI-compatible chat client (the ``OMS_LLM_*`` fallback)."""
...
def resolve(adapter: Any | None = None) -> Provider:
    """Resolve the model ``oms.distill`` curates through.

    Order (Settled): adapter ``distill_model()`` hook → ``OMS_LLM_*``
    OpenAI-compatible fallback → :class:`ProviderUnavailable`. ``oms.adapters``
    ...
    base_url = config.resolve("OMS_LLM_BASE_URL", "")
    model = config.resolve("OMS_LLM_MODEL", "")
    if base_url and model:
        return OpenAICompatibleProvider(
            base_url=base_url,
            model=model,
            api_key=config.resolve("OMS_LLM_API_KEY", ""),
        )

    raise ProviderUnavailable(
        "No distill model: an adapter must implement distill_model() or "
        "OMS_LLM_BASE_URL + OMS_LLM_MODEL must be configured (oms ships no keys)."
    )
