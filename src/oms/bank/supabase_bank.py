...
key (oms.bank.md "Access identities"). Network calls are retry-wrapped.
"""
...
_IDENTITY_KEY_VARS: dict[str, str] = {
    "public": "OMS_BANK_ANON_KEY",
    "trusted": "OMS_BANK_TRUSTED_KEY",
    "admin": "OMS_BANK_ADMIN_KEY",
    "curator": "OMS_BANK_CURATOR_KEY",
}


def _cf_access_headers() -> dict[str, str]:
    cid = config.resolve("OMS_BANK_CF_ACCESS_CLIENT_ID", "")
    secret = config.resolve("OMS_BANK_CF_ACCESS_CLIENT_SECRET", "")
    if cid and secret:
...
class SupabaseBank:
    """Async :class:`~oms.bank.base.Bank` over Supabase PostgREST."""
    ...
    def __init__(self, identity: str = "trusted") -> None:
        ...
        self._url = config.resolve("OMS_BANK_URL", config.OMS_BANK_URL)
        # identity@url so oms.core's hydration cache never confuses two Banks
