"""oms._mcp — the in-agent MCP server (M11).

One Python MCP server exposing oms's knowledge-loop verbs as tools any
...
gate — no string-parsing of y/n inside the chat (oms.web.md / advisor).

Run: ``python -m oms._mcp`` (the per-adapter installer registers this).
"""
...
from oms import setup_environment

setup_environment()  # ~/.oms/env then ./oms.env — Bank creds for the MCP child

app = FastMCP("oms")


# --------------------------------------------------------------------------- #
# session id resolution: OMS_SESSION env wins, else ~/.oms/active.
# This dual-source lets `oms <name>` thread the session at PTY-spawn time AND
# lets the agent (opened later/anywhere after one-time `oms start`) still work.
# --------------------------------------------------------------------------- #
...
def _session_id() -> str:
    ...
    if p.is_file():
        ...
        if s:
            return s
    raise RuntimeError(
        "no active oms session — run `oms start` first (writes ~/.oms/active), "
        "or export OMS_SESSION=<id> for this MCP server"
    )
...
async def discuss_draft(stance: str, packet: str | None = None) -> dict[str, Any]:
    ...
    from oms.bank import get_bank
    from oms.forum import retrieve
    from oms.forum.prompt import render_post_prompt
    ...
    if not ranked:
        return {
            "error": "no related posts to engage — call self_distill_draft first (retrieval-before-post; oms.forum)",
        }
...
async def commit_post(
    ...
    from oms.utils import sid as sid_
...
if __name__ == "__main__":  # pragma: no cover — `python -m oms._mcp`
    main()
