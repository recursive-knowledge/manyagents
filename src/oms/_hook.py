...
When the harness was launched by the oms wrapper, ``OMS_SESSION`` is in the
...
``$OMS_HOME/bindings/<session>.jsonl`` — the durable binding between an oms
...
When ``OMS_SESSION`` is absent (a normal, un-wrapped harness session) the
hook exits 0 immediately and writes nothing — installing it user-scope is
safe for the user's everyday sessions.

Constraints: **stdlib-only** (no oms imports — it must start fast in
...
``SessionStart``, if ``oms start`` stashed an allowed injection under
``$OMS_HOME/inject/<session>.json`` (the start-time inject offer,
2026-06-10), the hook emits the ``additionalContext`` JSON that Claude Code
folds into the conversation — this is how a bundle allowed at ``oms start``
...
def _bindings_dir() -> Path:
    return _oms_home() / "bindings"


def _injected_context(sid: str) -> str | None:
    """The additionalContext payload for an injection allowed at ``oms start``
    ...
    context; ``oms end`` removes the stash."""
    p = _oms_home() / "inject" / f"{sid}.json"
    if not p.is_file():
        return None
    ...
    if not isinstance(stash, dict) or not stash.get("bundle"):
        return None
    return (
        "Curated knowledge injected by oms for goal "
        f"'{stash.get('goal')}' (bundle {stash.get('packet_id')}; allowed by the "
        "user at `oms start`). Treat as prior constraints/insights from earlier "
