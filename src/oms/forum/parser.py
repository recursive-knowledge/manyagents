"""The mechanical post parser (oms.forum.md "Write-time discipline").
...
**hardened** for oms: swarms' ``evidence_post_id: int`` + ``task_id`` becomes
oms's ``evidence_ref``: a packet-id **string**, no task (the M6 analog of the
C3 swarms→oms ``Evidence`` mapping); plus oms-specific Bank-grounded checks
(``evidence_ref`` resolves, no-history hardening, quarantine refusal).

**C1 (oms.core.md:70/98; oms.forum.md:89):** :func:`parse_post` never
...
from oms.forum.schema import validate_schema
