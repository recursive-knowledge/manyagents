"""The 6-bucket evidence-grounded Insight schema (oms.distill.md "The Insight
schema", Open-Q §A1 resolved).

Ported from ``swarms/distillation/types.py:15-35`` + ``prompts.py:124-178`` and
**hardened** for oms (C3). The swarms→oms ``Evidence`` remap:

    swarms  ``{"task_id": "<id>", "post_id": <int>, "quote": "<verbatim>"}``
    oms     ``{"post_id": "<packet-id string>", "quote": "<verbatim>"}``

— a packet-id **string**, no ``task_id`` (oms has no task oracle; the
...
drop/cap/verbatim/boundary enforcement lives in ``oms.distill.parse`` (the C3
port-and-harden); the anti-meta enforcement code is imported from
``oms.forum.anti_meta`` so the curator filters against the *same code* the
agent wrote against.
"""
...
# first-class signal, not an afterthought (oms.distill.md:51).
BUCKETS: tuple[str, ...] = (
...
def empty_bundle() -> dict[str, list[dict[str, object]]]:
    ...
    buckets are correct and preferable to filler (oms.distill.md:53)."""
