...
enforce caps regardless of model output) and **hardens** it for oms, because
``oms.distill.md:53`` requires two checks that swarms enforced only at
...
Plus the swarms→oms ``Evidence`` remap (``schema.py``): ``post_id`` is a
...
parser uses (``oms.forum.anti_meta.has_banned_meta`` / ``is_concrete``) — the
...
A dated C3 Decision-log entry on ``oms.distill.md`` records this divergence
...
from oms.distill.schema import (
    BUCKETS,
    CONFIDENCE_LEVELS,
    MAX_CONDITION,
...
from oms.forum.anti_meta import has_banned_meta, is_concrete
...
def _clean_evidence(
    ...
    *,
    ...
    (the swarms→oms string-id ``allowed_post_ids`` remap) AND its ``quote`` is
...
def _clean_insight(
    ...
    *,
    ...
    if confidence not in CONFIDENCE_LEVELS:
        confidence = "low"
    # Recurrence promotion (mechanical weighting; oms.distill.md:80): a
