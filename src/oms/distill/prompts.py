"""The cache-split curator prompt (oms.distill.md:59; ports
...
``ANTI_META_BLOCK`` is imported from ``oms.forum`` and re-exported: it is the
...
(oms.forum.md / oms.distill.md "the anti-meta discipline").
...
from oms.distill.schema import BUCKETS
from oms.forum import ANTI_META_BLOCK, assert_anti_meta_rules_present
...
def build_distill_prompt(
    *,
    ...
    arrive already weighted/ordered (``oms.distill.weighting.weigh_posts``)."""
