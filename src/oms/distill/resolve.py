"""Hybrid curator resolution (oms.distill.md "Hybrid curator", settled).

``OMS_CURATOR_MODE`` ∈ ``local`` | ``server`` | ``auto``:

* **local** — the curator prompt runs on the user's own LLM (an installed
  adapter's headless ``distill_model()``, or the ``OMS_LLM_*``
  OpenAI-compatible fallback). OMA ships no inference for the user; the user
  pays; works with no server.
* **server** — the hosted corpus curator (``oms.distill.server``).
...
``oms.adapters.builtin._HeadlessModel.complete`` is blocking). The curator
...
class NoLocalCurator(RuntimeError):
    ...
    no ``OMS_LLM_*`` fallback configured)."""
    ...
    def __init__(self, model: Any) -> None:
        if model is None:
            raise NoLocalCurator(
                ...
                "or set OMS_LLM_BASE_URL/OMS_LLM_API_KEY/OMS_LLM_MODEL"
            )
        self._model = model
...
def _discover_local_model() -> Any | None:
    ...
    for name in available():
        ...
        if model is not None:
            return model
    if config.OMS_LLM_BASE_URL and config.OMS_LLM_API_KEY and config.OMS_LLM_MODEL:
        return _OpenAICompatModel(
            base_url=config.OMS_LLM_BASE_URL,
            api_key=config.OMS_LLM_API_KEY,
            model=config.OMS_LLM_MODEL,
        )
    return None


class _OpenAICompatModel:
    """Minimal OpenAI-compatible chat client (the ``OMS_LLM_*`` local
    ...
    def complete(self, prompt: str, *, max_tokens: int | None = None) -> str:
        ...
        resp = httpx.post(
            ...
            timeout=float(config.OMS_DISTILL_TIMEOUT_S),
        )
...
def resolve(
    ...
    *,
    ...
    """Select a curator. ``mode=None`` reads ``OMS_CURATOR_MODE``. ``model``
    (an object with ``.complete``) overrides local discovery (tests inject a
    fake here so the offline suite never shells out)."""
    selected = (mode or config.resolve("OMS_CURATOR_MODE", "auto")).strip().lower()
    url = server_url if server_url is not None else config.OMS_CURATOR_SERVER_URL
    ...
    raise ValueError(f"bad OMS_CURATOR_MODE {selected!r}; expected local|server|auto")
