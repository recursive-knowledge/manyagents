...
Binding tiers (Trace Renditions & Mining §4a): the ``oms._hook`` records
...
# Total artifact ceiling (mirrors oms.capture's OMS_TRACE_MAX_BYTES on the raw
...
def _bound_artifact(artifact: dict[str, Any]) -> None:
    ...
    Mirrors oms.capture's size-bound on the raw path so a rendition can never
