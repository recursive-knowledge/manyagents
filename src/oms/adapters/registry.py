"""Adapter discovery (oms.adapters.md "Registry resolution order — Settled").

Resolution order: a local install ``~/.oms/adapters/<name>/`` → the first-party
...
from oms.adapters.base import Adapter, AdapterError
...
def _local_root() -> Path:
    return Path(config.resolve("OMS_ADAPTERS_DIR", str(Path.home() / ".oms" / "adapters"))).expanduser()
...
def _load_local(name: str) -> type[Adapter] | None:
    ...
    if not init.is_file():
        return None
    spec = importlib.util.spec_from_file_location(f"oms._local_adapter_{name}", init)
