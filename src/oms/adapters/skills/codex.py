...
def _skill_body(verb: str, description: str) -> str:
    ...
    head = (
        ...
        f"Invoke as $oms-{verb} or by natural language describing the intent.\n---\n\n"
    )
    if verb == "self-distill":
        return (
            head
            + """\
# $oms-self-distill — emit one reflection post to the active oms session

Procedure (follow exactly):

1. Call the `oms.self_distill_draft` MCP tool (pass guidance from the user's request).
...
"""
    ...
    if verb == "discuss":
        return (
            head
            + """\
# $oms-discuss — emit one stance reply to a prior in-session post
...
"""
    ...
    if verb == "cross-distill":
        return (
            head
            + """\
# $oms-cross-distill — curate goal-scoped posts into a bundle

Procedure:

1. Call `oms.cross_distill`. The curator runs in the background.
2. If the tool returns `{"ok": false, "error": "Run /self-distill first!"}`, tell the user to run $oms-self-distill first and STOP.
...
"""
    ...
    if verb == "inject":
        return (
            head
            + """\
# $oms-inject — preview a bundle and (on accept) record an injection
...
def _mcp_server_value() -> dict[str, Any]:
    """``[mcp_servers.oms]`` table value. Codex MCP servers don't get full
    env passthrough — declare ``env_vars`` allowlist explicitly."""
    return {
        "command": sys.executable,
        "args": ["-m", "oms._mcp"],
        "env_vars": ["OMS_SESSION", "OMS_HOME"],
...
def build_plan(*, session_id: str | None, oma_home: Path | None = None, scope: str = "user") -> InstallPlan:
    ...
    # so the path isn't used here. Gemini's bundle is oms-owned and needs it.
    root = _root(scope)
    ops: list[FileOp] = []
    ...
    # The MCP server entry under [mcp_servers.oms] — one MERGE op.
    ops.append(
        FileOp(
            kind="merge",
            path=config_path,
            payload={"__section__": "mcp_servers.oms", "__value__": _mcp_server_value()},
            description="Register the oms MCP server under `[mcp_servers.oms]` (preserves all other servers + comments via tomlkit).",
            merge_keys=("mcp_servers.oms",),
        )
    )
    ...
    for tool in ("commit_post", "inject_commit"):
        ops.append(
            FileOp(
                kind="merge",
                path=config_path,
                payload={"__section__": f"mcp_servers.oms.tools.{tool}", "__value__": _tool_approval_value()},
