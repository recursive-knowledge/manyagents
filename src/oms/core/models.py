...
Explicit async ``.fetch()`` hydration (cache → oms.bank). A bare ``Packet(...)``
...
from oms.core.collection import Collection
...
class Goal(BaseModel):
    ...
    oracle. Never gates anything (oms.core.md)."""
...
class Agent(BaseModel):
    ...
    # Derived activity span for the oms.web ``GET /s/{session}/agents`` route
    # (oms.web.md). Defaulted None; populated only by :meth:`from_activity`.
    ...
    def from_activity(cls, row: dict[str, Any], *, packets: Iterable[dict[str, Any]] = ()) -> Agent:
        ...
        ``oms.web`` route fetches the agent + packet rows and hands them here,
        so the route stays a dumb orchestrator and the derivation lives in the
        frozen model (oms.web.md; Design Principles §3, §4).
...
class _PacketFields(BaseModel):
    ...
    def _check_shape(self) -> _PacketFields:
        ...
        if self.preference is not None and self.type != "distill":
            # C1 (oms.core.md:70/98): preference=accept|reject is distill-only,
            ...
            # never carry preference. Mechanical, past the oms.forum parser.
            raise ValueError("preference is distill-only (oms.core.md; C1) — a post never carries it")
        return self


class KnowledgePacket(_PacketFields):
    """The canonical public wire shape (oms.web). Mirrors Packet's public
