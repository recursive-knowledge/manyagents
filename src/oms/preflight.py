...
(M8; oms.cli.md "preflight.py").

Three checks, fail-fast with a specific message:
  1. env presence — ``OMS_BANK_URL`` and at least one Bank key.
  ...
  3. migration inventory — the ``supabase/migrations/*.sql`` files oms.bank
...
_BANK_KEYS = ("OMS_BANK_ANON_KEY", "OMS_BANK_TRUSTED_KEY", "OMS_BANK_ADMIN_KEY", "OMS_BANK_CURATOR_KEY")
_MIGRATIONS_DIR = Path(__file__).resolve().parents[2] / "supabase" / "migrations"


def _check_env() -> str | None:
    if not os.environ.get("OMS_BANK_URL"):
        return "OMS_BANK_URL is unset (point it at the Supabase PostgREST URL)"
...
def _live_schema_diff(url: str) -> str:
    ...
    anon grant, so without ``OMS_BANK_ADMIN_KEY`` this is skipped, not failed —
    the offline / non-privileged path is byte-for-byte unchanged."""
    admin = os.environ.get("OMS_BANK_ADMIN_KEY")
    if not admin:
        return "skipped (no OMS_BANK_ADMIN_KEY; schema_migrations needs the privileged role)"
