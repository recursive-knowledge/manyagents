"""Size bounding — *the remaining genuinely hard problem* (oms.capture.md
"Trace size vs. context window — Open (highest-risk)").

A long session exceeds any local model's context, so a budget
(``OMS_TRACE_MAX_BYTES``) is enforced *here*, not in ``oms.distill`` (which
...
def bound(trace: CanonicalTrace, *, max_bytes: int | None = None) -> CanonicalTrace:
    ...
    ``bytes_out`` stamped)."""
    budget = max_bytes if max_bytes is not None else config.OMS_TRACE_MAX_BYTES
