...
oms.capture.md "Conformance is the adapter author's job — Settled": there is
**no** central PTY-parsing heuristic. The adapter maps its agent to
``CanonicalTrace`` and declares ``source_fidelity``; ``oms.capture`` validates
...
from oms.capture.models import EVENT_KINDS, SOURCE_FIDELITIES, CanonicalTrace
...
def validate(trace: CanonicalTrace) -> CanonicalTrace:
    ...
    if "/" in trace.session_id:
        ...
        # session_id would break Packet.session_id derivation (oms.core).
