"""oms.capture — raw agent session → normalized, bounded, scrubbed, replayable
``raw`` Packet (M4; oms.capture.md).

*The invisible prerequisite* (Design Principles §2): ``oms.distill`` cannot
...
from oms.capture.bound import bound
from oms.capture.conformance import ConformanceError, validate
from oms.capture.models import CanonicalTrace, ScrubReport, TraceEvent
from oms.capture.scrub import SCRUB_VERSION, scrub
...
async def persist(trace: CanonicalTrace, *, bank: Bank | None = None, complete: bool = True) -> str:
    ...
    a ``traces`` row in ``oms.bank``).

    ``complete`` records whether *capture itself* finished (a session can be
    killed mid-stream — oms.capture.md "Operations & recovery"); it is distinct
