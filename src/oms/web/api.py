...
1. **Trace-body exposure is a single explicit switch, enforced at two
   ...
   are public — ``OMS_WEB_PUBLIC_RAW`` defaults on, and migration ``00008``
   grants anon SELECT on ``traces`` (the captured trajectory is the product
   being demonstrated; ``oms.capture`` scrubs before persist). Setting
   ``OMS_WEB_PUBLIC_RAW=0`` restores the original M9 stance for the app layer
...
# Identities that always read raw trace bodies (oms.web.md "Trace/PII").
# The public identity reads them too while OMS_WEB_PUBLIC_RAW is on (pre-alpha
...
def _public_raw_enabled() -> bool:
    return config.resolve("OMS_WEB_PUBLIC_RAW", config.OMS_WEB_PUBLIC_RAW).strip().lower() not in _FALSEY
...
def create_app(*, bank: Bank | None = None, identity: str = "public") -> FastAPI:  # noqa: C901 — N route closures inherently bump cyclomatic complexity
    ...
    async def session_view(
        ...
        if p is not None:
            ...
            # documented leak-recovery seam — oms.capture scrub docstring);
