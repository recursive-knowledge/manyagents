...
Lets ``manyagent.distill`` use the *user's* model without ``manyagent`` hosting inference;
``manyagent`` ships no keys. Resolution order (Settled): adapter ``distill_model()``
hook → configured ``MANYAGENT_LLM_*`` OpenAI-compatible fallback → hard error
(never a silent skip; asserted in ``manyagent.distill``).

Rate-limit detection: the general per-provider signal map is Open
(``manyagent.utils.md``). The two datasmith-proven schemas (Codex free-text reset,
Claude structured ``rate_limit_event``) are ported here verbatim because
``manyagent.utils.md`` Verification requires parsing canned Codex/Claude payloads;
...
def rate_limit_signal(raw_error: str, *, provider: str | None = None) -> RateLimit | None:
    ...
    The general per-provider map is Open (``manyagent.utils.md``).
    """
    if not raw_error:
        return None
...
class Provider(Protocol):
    """The model seam ``manyagent.distill`` curates through."""
...
class ProviderUnavailable(RuntimeError):
    ...
    configured ``MANYAGENT_LLM_*`` fallback (``manyagent`` ships no keys)."""


@dataclass
class OpenAICompatibleProvider:
    """Minimal OpenAI-compatible chat client (the ``MANYAGENT_LLM_*`` fallback)."""

    base_url: str
    model: str
    api_key: str = ""
    name: str = "manyagent-llm"
...
def resolve(adapter: Any | None = None) -> Provider:
    """Resolve the model ``manyagent.distill`` curates through.

    Order (Settled): adapter ``distill_model()`` hook → ``MANYAGENT_LLM_*``
    OpenAI-compatible fallback → :class:`ProviderUnavailable`. ``manyagent.adapters``
    ...
    base_url = config.resolve("MANYAGENT_LLM_BASE_URL", "")
    model = config.resolve("MANYAGENT_LLM_MODEL", "")
    if base_url and model:
        return OpenAICompatibleProvider(
            base_url=base_url,
            model=model,
            api_key=config.resolve("MANYAGENT_LLM_API_KEY", ""),
        )

    raise ProviderUnavailable(
        "No distill model: an adapter must implement distill_model() or "
        "MANYAGENT_LLM_BASE_URL + MANYAGENT_LLM_MODEL must be configured (manyagent ships no keys)."
    )
