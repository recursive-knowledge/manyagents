...
Async because ``manyagent.core.Packet.fetch()`` is async (manyagent.core.md) and ``manyagent.web``
is FastAPI. The Bank trades plain ``dict`` records (the PostgREST row shape);
``manyagent.core`` (M3) layers the frozen Pydantic ``Packet`` on top — so the Bank
...
# Identity selects the access key (manyagent.bank.md 4-identity model).
...
class Bank(Protocol):
    ...
    # Stable per-Bank identity so manyagent.core's hydration cache never confuses two
    ...
    # Every agents row carrying ``principal_id`` — the operator's persistent
    # cross-goal identity (00011). Spans sessions/goals; empty for NULL principals.
