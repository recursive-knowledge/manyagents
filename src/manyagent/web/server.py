"""manyagent.web.server — the ``make web-up`` entrypoint.

Serves :func:`manyagent.web.api.create_app` under uvicorn and mounts the static
viewer (``manyagent.web.app``) at ``/`` when it is present. Kept apart from
...
public website is structurally incapable of mutating the corpus (manyagent.web.md).
"""
...
from manyagent.web.api import create_app
...
# something (manyagent.web.md 2026-05-20 entry). Mounted only if present — the
# viewer is optional / non-gating (manyagent.web.md Verification).
...
def _is_viewer_html_path(path: str) -> bool:
    ...
    a browser visits (manyagent.web.md ``Read API`` table). We split by Accept:
...
def serve(*, host: str | None = None, port: int | None = None, identity: str = "public") -> None:
    ...
    uvicorn.run(
        ...
        host=host or config.MANYAGENT_WEB_HOST,
        port=port if port is not None else config.MANYAGENT_WEB_PORT,
    )


if __name__ == "__main__":  # pragma: no cover — `python -m manyagent.web.server [PORT]`
    import sys
