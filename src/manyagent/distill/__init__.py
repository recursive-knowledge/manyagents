"""manyagent.distill — the **curator** (M7; manyagent.distill.md).

Reads goal-scoped ``post`` packets (manyagent.forum) and emits a ``distill`` packet:
...
* **6-bucket Insight schema** (``schema``) — swarms ``types.py`` ported, the
  ``Evidence`` remapped to manyagent packet-id **strings** (no ``task_id``).
* **Mechanical validation, port + harden (C3)** (``parse``) — swarms'
  *mechanical-not-trusted-to-the-model* ``_as_insight_list`` ported, plus the
  two checks manyagent.distill.md:53 requires be mechanical (the ``always``/``n/a``
...
from manyagent.distill.curator import CurationError, NoPostsError, curate
from manyagent.distill.parse import validate_bundle
from manyagent.distill.prompts import ANTI_META_BLOCK, build_distill_prompt
from manyagent.distill.resolve import Curator, NoLocalCurator, resolve
from manyagent.distill.schema import BUCKETS, empty_bundle
