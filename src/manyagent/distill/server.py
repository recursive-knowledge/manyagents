"""The hosted-curator stub (manyagent.distill.md "Hybrid curator" — server mode).
...
manyagent.bank dependency, but the import stays out of the offline hot path.
"""
...
class ServerUnavailable(RuntimeError):
    ...
    back to the local curator (manyagent.distill.md: 'usable degraded')."""
...
class ServerCurator:
    ...
    async def complete(self, system: str, user: str) -> str:
        if not self._url:
            raise ServerUnavailable("MANYAGENT_CURATOR_SERVER_URL is unset; no hosted curator")
        try:
            import httpx
