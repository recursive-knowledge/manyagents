...
"""manyagent.testing — simulated-conversation scaffolding: dummy Bank, dummy model,
dummy agent harness.

The unit of test this module enables is a **conversation**, not a function
call: drive the *real* session-lifecycle verbs (``manyagent.cli._do_*``) and the
*real* knowledge-loop handlers (``manyagent._handlers.do_*``) end to end, with only
the three seams a live run would shell out to replaced by scripted doubles —

* the Bank        → :class:`manyagent.bank.FakeBank` (in-memory, already shipped)
...
Typical use (see ``tests/test_testing.py``)::
    ...
    with Simulation(home=tmp_path / ".manyagent") as sim:
...
from manyagent.capture import SCRUB_VERSION, CanonicalTrace, TraceEvent
...
async def seed_trial_story(bank: FakeBank) -> dict[str, str]:
    ...
    retrieval, quarantine, manyagent.web) that start from existing knowledge rather
...
class Simulation:
    ...
    ``MANYAGENT_HOME`` at a private dir (so the active-session file never touches
    the real ``~/.manyagent``), sets ``MANYAGENT_INSTALL_SKILLS=deny``, and clears the
    ...
    def __enter__(self) -> Simulation:
        from manyagent import _handlers as handlers_mod
        from manyagent import cli as cli_mod
        ...
        # `manyagent.distill.resolve` the *name* is the re-exported function; the
        # submodule must come from the import system (the test_e2e trap).
        resolve_mod = importlib.import_module("manyagent.distill.resolve")

        if self._home is None:
            self._home = Path(tempfile.mkdtemp(prefix="manyagent-sim-"))
        # MANYAGENT_CURATOR_MODE=local: resolve() re-reads it from env at call time,
        # so a dev's manyagent.env setting `server`/`auto` would otherwise route
        # cross_distill around the patched local-model seam to a REAL server.
        env_pins = (
            ("MANYAGENT_HOME", str(self._home)),
            ("MANYAGENT_INSTALL_SKILLS", "deny"),
            ("MANYAGENT_CURATOR_MODE", "local"),
        )
        for key, value in env_pins:
            self._saved_env[key] = os.environ.get(key)
            os.environ[key] = value
        for key in ("MANYAGENT_SESSION", "MANYAGENT_NONINTERACTIVE"):
