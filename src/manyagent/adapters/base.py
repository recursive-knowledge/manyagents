"""The ``Adapter`` ABC — the whole contract (manyagent.adapters.md "The four-method
contract — Settled").
...
normalization/bounding/secret-scrub is ``manyagent.capture``'s job — keeping adapters
...
lifecycle stage of ``manyagent.capture.CanonicalTrace``. The adapter author emits
``CanonicalTrace``-shaped material (``manyagent.capture.md`` "Validate conformance");
``manyagent.capture`` validates → scrubs → bounds → persists. The ABC keeps the
frozen ``capture() -> RawTrace`` wording via this alias (see the
2026-05-19 Decision-log entry on ``manyagent.adapters.md``).
...
from manyagent.capture import CanonicalTrace
...
class Adapter(ABC):
    ...
    def capture(self) -> RawTrace:
        ...
        size-bounding, and persistence are NOT done here — ``manyagent.capture``
    ...
    def distill_model(self) -> object | None:
        """Optional: expose this agent's own model to ``manyagent.distill`` (the
        ``manyagent.utils.provider`` seam). Default: no model → caller falls back to
    ...
    def mine(self, ctx: MineContext) -> dict[str, Any] | None:
        ...
        delegate to ``manyagent.adapters.miners.<name>.mine`` (the skills pattern)."""
        return None
...
class MineContext:
    ...
    ``bindings`` are the ``manyagent._hook`` lifecycle records for this run
...
class PromptPrefixInjector:
    """The lowest-common-default ``inject``/``retrieve`` (manyagent.adapters.md
