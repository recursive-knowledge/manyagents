...
event carrying the tee. ``manyagent.distill`` must degrade gracefully on ``"pty"``
...
from manyagent.adapters.base import Adapter, PromptPrefixInjector, RawTrace
from manyagent.adapters.builtin import _HeadlessModel, _make_trace, _read_source
from manyagent.capture import TraceEvent
...
class GeminiAdapter(PromptPrefixInjector, Adapter):
    ...
    def install_skills(
        ...
        *,
        ...
        """Stage the gemini-extension bundle under ``$MANYAGENT_HOME/extensions/
        gemini-manyagent/`` and ``gemini extensions link`` it (symlink semantics →
        ``manyagent`` pip-upgrade auto-propagates the bundle). Idempotent via
        pre-uninstall."""
