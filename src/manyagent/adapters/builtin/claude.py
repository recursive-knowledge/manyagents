...
"structured"`` (manyagent.adapters.md "Capture fidelity per agent — Settled").

Maps Claude Code ``--output-format stream-json`` events to ``TraceEvent``s.
The author owns this mapping (the contract); ``manyagent.capture`` validates it.
"""
...
from manyagent.adapters.builtin import _jsonl, _msg_text, _StructuredBuiltin
...
class ClaudeAdapter(_StructuredBuiltin):
    ...
    def mine(self, ctx: MineContext) -> dict[str, Any] | None:
        ...
        transcript(s) bound to this run (manyagent.adapters.miners.claude)."""
        from manyagent.adapters.miners.claude import mine
