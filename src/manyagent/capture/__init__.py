"""manyagent.capture — raw agent session → normalized, bounded, scrubbed, replayable
``raw`` Packet (M4; manyagent.capture.md).

*The invisible prerequisite* (Design Principles §2): ``manyagent.distill`` cannot
...
from manyagent.capture.bound import bound
from manyagent.capture.conformance import ConformanceError, validate
from manyagent.capture.models import CanonicalTrace, ScrubReport, TraceEvent
from manyagent.capture.scrub import SCRUB_VERSION, scrub
...
async def persist(trace: CanonicalTrace, *, bank: Bank | None = None, complete: bool = True) -> str:
    ...
    a ``traces`` row in ``manyagent.bank``).

    ``complete`` records whether *capture itself* finished (a session can be
    killed mid-stream — manyagent.capture.md "Operations & recovery"); it is distinct
