"""Size bounding — *the remaining genuinely hard problem* (manyagent.capture.md
"Trace size vs. context window — Open (highest-risk)").

A long session exceeds any local model's context, so a budget
(``MANYAGENT_TRACE_MAX_BYTES``) is enforced *here*, not in ``manyagent.distill`` (which
...
def bound(trace: CanonicalTrace, *, max_bytes: int | None = None) -> CanonicalTrace:
    ...
    ``bytes_out`` stamped)."""
    budget = max_bytes if max_bytes is not None else config.MANYAGENT_TRACE_MAX_BYTES
