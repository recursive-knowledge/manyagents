"""The ``CanonicalTrace`` contract (manyagent.capture.md "CanonicalTrace").
...
``manyagent.capture``. Pipeline stages treat them as effectively-immutable and
...
# The five event kinds an adapter may emit (manyagent.capture.md "CanonicalTrace").
...
class CanonicalTrace:
    ...
    first-class: ``manyagent.distill`` must degrade gracefully on ``"pty"`` and never
    assume tool-call structure exists (manyagent.capture.md)."""
