"""Hybrid curator resolution (oma.distill.md "Hybrid curator", settled).

``OMA_CURATOR_MODE`` ∈ ``local`` | ``server`` | ``auto``:

* **local** — the curator prompt runs on the user's own LLM (an installed
  adapter's headless ``distill_model()``, or the ``OMA_LLM_*``
  OpenAI-compatible fallback). OMA ships no inference for the user; the user
  pays; works with no server.
* **server** — the hosted corpus curator (``oma.distill.server``).
...
``oma.adapters.builtin._HeadlessModel.complete`` is blocking). The curator
...
from oma.distill.server import ServerCurator, ServerUnavailable
...
class NoLocalCurator(RuntimeError):
    ...
    no ``OMA_LLM_*`` fallback configured)."""
    ...
    def __init__(self, model: Any) -> None:
        if model is None:
            raise NoLocalCurator(
                ...
                "or set OMA_LLM_BASE_URL/OMA_LLM_API_KEY/OMA_LLM_MODEL"
            )
        self._model = model
...
def _discover_local_model() -> Any | None:
    ...
    for name in available():
        ...
        if model is not None:
            return model
    if config.OMA_LLM_BASE_URL and config.OMA_LLM_API_KEY and config.OMA_LLM_MODEL:
        return _OpenAICompatModel(
            base_url=config.OMA_LLM_BASE_URL,
            api_key=config.OMA_LLM_API_KEY,
            model=config.OMA_LLM_MODEL,
        )
    return None


class _OpenAICompatModel:
    """Minimal OpenAI-compatible chat client (the ``OMA_LLM_*`` local
    ...
    def complete(self, prompt: str, *, max_tokens: int | None = None) -> str:
        ...
        resp = httpx.post(
            ...
            timeout=float(config.OMA_DISTILL_TIMEOUT_S),
        )
...
def resolve(
    ...
    *,
    ...
    """Select a curator. ``mode=None`` reads ``OMA_CURATOR_MODE``. ``model``
    (an object with ``.complete``) overrides local discovery (tests inject a
    fake here so the offline suite never shells out)."""
    selected = (mode or config.resolve("OMA_CURATOR_MODE", "auto")).strip().lower()
    url = server_url if server_url is not None else config.OMA_CURATOR_SERVER_URL
    ...
    raise ValueError(f"bad OMA_CURATOR_MODE {selected!r}; expected local|server|auto")
