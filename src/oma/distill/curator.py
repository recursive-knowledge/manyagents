"""The ``/cross-distill`` curator state machine (oma.distill.md "State
...
3. curator = resolve(OMA_CURATOR_MODE)         # local | server | auto
...
**Idempotency-key refinement (M7 build decision; logged on oma.distill.md):**
...
from oma.distill.parse import validate_bundle
from oma.distill.prompts import build_distill_prompt
from oma.distill.resolve import Curator, resolve
...
class NoPostsError(RuntimeError):
    ...
    the CLI surfaces (oma.distill.md Verification: ``"Run /self-distill
    first!"``)."""
...
async def curate(
    *,
    ...
    win = window_days if window_days is not None else config.OMA_CROSSDISTILL_WINDOW_DAYS
    ...
    # and silently overwriting it violates oma.bank.md:92 (append-only); the
    # consumer decides exclusion (oma.core.md: quarantine is non-hiding;
    ...
    # excluded post changes the parent set ⇒ a different pid (oma.bank.md
