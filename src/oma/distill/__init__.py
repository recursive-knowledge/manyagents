"""oma.distill — the **curator** (M7; oma.distill.md).

Reads goal-scoped ``post`` packets (oma.forum) and emits a ``distill`` packet:
...
* **6-bucket Insight schema** (``schema``) — swarms ``types.py`` ported, the
  ``Evidence`` remapped to oma packet-id **strings** (no ``task_id``).
* **Mechanical validation, port + harden (C3)** (``parse``) — swarms'
  *mechanical-not-trusted-to-the-model* ``_as_insight_list`` ported, plus the
  two checks oma.distill.md:53 requires be mechanical (the ``always``/``n/a``
...
from oma.distill.curator import CurationError, NoPostsError, curate
from oma.distill.parse import validate_bundle
from oma.distill.prompts import ANTI_META_BLOCK, build_distill_prompt
from oma.distill.resolve import Curator, NoLocalCurator, resolve
from oma.distill.schema import BUCKETS, empty_bundle
