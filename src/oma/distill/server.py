"""The hosted-curator stub (oma.distill.md "Hybrid curator" — server mode).

A `curator` identity in oma.bank's role model periodically re-distills the
...
oma.bank dependency, but the import stays out of the offline hot path.
"""
...
class ServerUnavailable(RuntimeError):
    ...
    back to the local curator (oma.distill.md: 'usable degraded')."""
...
class ServerCurator:
    ...
    async def complete(self, system: str, user: str) -> str:
        if not self._url:
            raise ServerUnavailable("OMA_CURATOR_SERVER_URL is unset; no hosted curator")
        try:
            import httpx
