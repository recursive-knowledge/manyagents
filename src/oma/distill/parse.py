...
enforce caps regardless of model output) and **hardens** it for oma, because
``oma.distill.md:53`` requires two checks that swarms enforced only at
...
Plus the swarms→oma ``Evidence`` remap (``schema.py``): ``post_id`` is a
...
parser uses (``oma.forum.anti_meta.has_banned_meta`` / ``is_concrete``) — the
...
A dated C3 Decision-log entry on ``oma.distill.md`` records this divergence
...
from oma.distill.schema import (
    BUCKETS,
    CONFIDENCE_LEVELS,
    MAX_CONDITION,
...
from oma.forum.anti_meta import has_banned_meta, is_concrete
...
def _clean_evidence(
    ...
    *,
    ...
    (the swarms→oma string-id ``allowed_post_ids`` remap) AND its ``quote`` is
...
def _clean_insight(
    ...
    *,
    ...
    if confidence not in CONFIDENCE_LEVELS:
        confidence = "low"
    # Recurrence promotion (mechanical weighting; oma.distill.md:80): a
