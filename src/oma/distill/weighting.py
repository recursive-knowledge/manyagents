"""Outcome weighting — the swarms ``native_score`` replacement (oma.distill.md
...
are never compared; there is no global numeric ranking (oma.distill.md:80).
"""
...
async def weigh_posts(posts: list[dict[str, Any]], *, bank: Bank) -> list[dict[str, Any]]:
    ...
    for p in posts:
        ...
        # OMA_REUSE_WEIGHT is the one knob a user reaches for to tune the
        # load-bearing default signal (oma.core.md "reuse is the load-bearing
        # default"); applied here so it flows into both ordering and the
        # rendered prompt hint (Design Principles §8 — no dead OMA_ knob).
        reuse = float(r.get("reuse_score", 0.0) or 0.0) * config.OMA_REUSE_WEIGHT
