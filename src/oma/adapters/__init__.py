"""oma.adapters — the agent adapter/plugin system (M5; oma.adapters.md).
...
``oma.capture``'s job (Design Principles §2). The contract is the ``Adapter``
ABC; discovery is ``registry.resolve()`` (local → builtin → hub, Settled
order). The builtins live under ``oma.adapters.builtin.*`` and are *not*
flattened onto the top-level surface (only ``Adapter`` is — the key noun).
"""

from __future__ import annotations

from oma.adapters.base import (
    Adapter,
    AdapterError,
    NotInstalled,
...
from oma.adapters.registry import available, resolve
