...
event carrying the tee. ``oma.distill`` must degrade gracefully on ``"pty"``
...
from oma.adapters.base import Adapter, PromptPrefixInjector, RawTrace
from oma.adapters.builtin import _HeadlessModel, _make_trace, _read_source
from oma.capture import TraceEvent
...
class GeminiAdapter(PromptPrefixInjector, Adapter):
    ...
    def install_skills(
        ...
        *,
        ...
        """Stage the gemini-extension bundle under ``$OMA_HOME/extensions/
        gemini-oma/`` and ``gemini extensions link`` it (symlink semantics →
        ``oma`` pip-upgrade auto-propagates the bundle). Idempotent via
        pre-uninstall."""
