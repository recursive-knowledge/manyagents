...
"structured"`` (oma.adapters.md "Capture fidelity per agent — Settled").

Maps Claude Code ``--output-format stream-json`` events to ``TraceEvent``s.
The author owns this mapping (the contract); ``oma.capture`` validates it.
"""
...
from oma.adapters.builtin import _jsonl, _msg_text, _StructuredBuiltin
...
class ClaudeAdapter(_StructuredBuiltin):
    ...
    def install_skills(
        ...
        *,
        ...
        """Drop oma's skill files + register the MCP server entry so the user
        can type ``/self-distill`` (etc.) inside Claude Code. Idempotent;
        every write is logged in ``$OMA_HOME/installed/claude.json`` and
        ``oma uninstall claude`` reverses it cleanly (oma.adapters.skills.claude)."""
