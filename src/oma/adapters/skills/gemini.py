...
- ``user``: ``~/.gemini/extensions/oma/`` — a single extension bundle
...
- ``project``: ``<cwd>/.gemini/extensions/oma/`` (same bundle, scoped to
...
We declare ``OMA_SESSION`` there. The MCP server also falls back to
``~/.oma/active`` (set by ``oma start``) so the chain works either way.
"""
...
This extension lets the user run `oma`'s knowledge-curation loop from inside
Gemini CLI. The four slash commands (`/self-distill`, `/discuss`,
`/cross-distill`, `/inject`) are defined in `commands/` and call into the
`oma` MCP server registered in `gemini-extension.json`.

## The contract

- The active oma session is set by `oma start` (writes `~/.oma/active`);
  `OMA_SESSION` env wins if set. If neither is present, the MCP tool errors
  and you should tell the user to run `oma start` first.
- For `/self-distill` and `/discuss`: always call the corresponding `*_draft`
  ...
  + ★, and **only on accept** call `mcp__oma__commit_post`. On reject, do
  NOT call commit_post — the Bank stays untouched (the C1 invariant: no
  persistence without explicit consent).
- For `/inject`: always call `mcp__oma__inject_preview` first, show the
  preview, ask for accept, and **only on accept** call `mcp__oma__inject_commit`.
...
def _toml_command(verb: str) -> str:
    ...
    if verb == "self-distill":
        return """\
description = "Draft and (on accept) commit ONE reflection post to the active oma session."
prompt = \"\"\"
Follow this procedure exactly for /self-distill:

1. Call `mcp__oma__self_distill_draft` (pass `guidance={{args}}` if there is text after the slash).
...
5. **Only on accept**, call `mcp__oma__commit_post(kind='reflection', structured={...}, rating=N)`. Gemini's permission UI will fire — the user must approve before the post is persisted.
...
2. Call `mcp__oma__discuss_draft(stance=..., packet=...)`.
...
6. **Only on accept**, call `mcp__oma__commit_post(kind='reply', structured={...}, reply_to=..., stance=...)`. The permission UI will fire.
...
1. Call `mcp__oma__cross_distill`. The curator runs in the background.
...
1. Call `mcp__oma__inject_preview(packet={{args}} or null)`.
...
5. **Only on accept**, call `mcp__oma__inject_commit(packet=<id>)`. Gemini's permission UI will fire — the user must approve again before the ledger row is written.
...
def _manifest_payload() -> dict[str, Any]:
    ...
    ``OMA_SESSION`` env-passthrough allowlist (gemini extensions have
    ...
    return {
        "name": "oma",
        "version": "0.1.0",
        "description": "Oh My Agent — in-agent knowledge curation. Type /self-distill, /discuss, /cross-distill, /inject inside Gemini CLI.",
        "mcpServers": {
            "oma": {
        ...
        "settings": [
            {
                "name": "session",
                "description": "Active oma session id (set by `oma start`; the MCP server also falls back to ~/.oma/active).",
                "envVar": "OMA_SESSION",
                "sensitive": False,
            }
        ],
...
def _bundle_root(oma_home: Path, scope: str) -> Path:
    """Source of truth for the extension bundle is **oma-owned** (under
    ``$OMA_HOME``), not the gemini extensions dir directly. ``gemini extensions
    link`` then symlinks it into ``~/.gemini/extensions/oma/`` — the
    documented register path, and updates to the source propagate without
    re-install. Project scope uses ``<cwd>/.oma/extensions/gemini-oma/`` so the
    bundle travels with the repo if the user wants per-project skills."""
    if scope == "user":
        return oma_home / "extensions" / "gemini-oma"
    if scope == "project":
        return Path.cwd() / ".oma" / "extensions" / "gemini-oma"
...
def _gemini_cli_actions(bundle_path: Path) -> list[CLIAction]:
    ...
    which ``link`` doesn't, so we can declare the formal ``OMA_SESSION``
    ...
    uninstall = ("gemini", "extensions", "uninstall", "oma")
    return [
        CLIAction(
            install_argv=uninstall,
            uninstall_argv=("true",),  # pre-clear's inverse is a no-op
            description="pre-clear any existing oma gemini extension (install isn't idempotent)",
        ),
        CLIAction(
            ...
            description="register the oma extension with Gemini CLI",
            stdin_input="1\n",
        ),
    ]
...
def build_plan(*, session_id: str | None, oma_home: Path, scope: str = "user") -> InstallPlan:
    ...
    ops: list[FileOp] = [
        FileOp(
            ...
            description="Extension manifest — declares the `oma` MCP server and `OMA_SESSION` env passthrough.",
        ),
        FileOp(
            kind="create",
            path=root / "GEMINI.md",
    ...
    # Pre-trust the oma-owned bundle folder so `gemini extensions install`
