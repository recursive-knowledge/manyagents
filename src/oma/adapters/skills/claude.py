...
- ``user``: ``~/.claude/skills/oma-<verb>/SKILL.md`` (4 files) + MERGE the
  ``mcpServers.oma`` entry into ``~/.claude/settings.json``. The user types
  ``/self-distill`` natively inside Claude Code.
- ``project``: ``<cwd>/.claude/skills/oma-<verb>/SKILL.md`` + MERGE
  ``<cwd>/.mcp.json``. Scoped to the repo the wrapper was launched in.

The MCP server command uses ``sys.executable`` (the python `oma` itself is
running under) so the agent's spawned server matches the install, even
inside a uv-managed venv. ``OMA_SESSION`` is **not** baked into the config
...
_VERBS: tuple[tuple[str, str, str], ...] = (
    (
        "self-distill",
        "Draft and (on accept) commit ONE reflection post to the active oma session.",
        "self_distill_draft",
    ),
...
---
name: self-distill
description: Draft and (on accept) commit ONE evidence-grounded reflection post to the active oma session.
disable-model-invocation: true
allowed-tools:
  - mcp__oma__{draft_tool}
---

# /self-distill — emit one reflection post to the active oma session

Follow this procedure exactly:

1. Call `mcp__oma__self_distill_draft` (pass `guidance=$ARGUMENTS` if the user supplied any).
...
5. **Only on accept**, call `mcp__oma__commit_post` with `kind="reflection"`, the structured payload, and the chosen rating. Claude Code's permission prompt will fire — the user must explicitly approve before the post is persisted.
6. On reject, **do NOT call commit_post** — the Bank stays untouched (C1).

The active oma session is auto-detected from `$OMA_SESSION` or `~/.oma/active`; if neither is set the MCP tool errors and you should tell the user to run `oma start` first.
"""
    if verb == "discuss":
        return f"""\
---
...
2. Call `mcp__oma__discuss_draft` with `stance=...` and `packet=...` (the @-stripped id, or null).
...
6. **Only on accept**, call `mcp__oma__commit_post` with `kind="reply"`, the structured payload, `reply_to=<from draft>`, `stance=<from draft>`. The permission prompt will fire.
...
1. Call `mcp__oma__cross_distill`. The curator runs in the background (uses `OMA_LLM_*` config or an installed agent CLI).
...
1. Call `mcp__oma__inject_preview` with `packet=$ARGUMENTS` (or null).
...
5. **Only on accept**, call `mcp__oma__inject_commit` with the same packet id. Claude Code's permission prompt will fire — the user must approve again before the ledger row is written.
...
def _mcp_cli_actions(scope: str) -> list[CLIAction]:
    """Register the oma MCP server via the official ``claude mcp`` CLI — the
    ...
    add = ("claude", "mcp", "add", "--scope", cli_scope, "oma", "--", sys.executable, "-m", "oma._mcp")
    remove = ("claude", "mcp", "remove", "--scope", cli_scope, "oma")
    return [
        ...
        CLIAction(
            install_argv=remove,
            uninstall_argv=("true",),
            description=f"pre-clear any existing oma MCP server (--scope {cli_scope})",
        ),
        CLIAction(
            install_argv=add,
            uninstall_argv=remove,
            description=f"register the oma MCP server with Claude Code (--scope {cli_scope})",
        ),
    ]
...
def build_plan(*, session_id: str | None, oma_home: Path | None = None, scope: str = "user") -> InstallPlan:
    ...
    (``oma.adapters.skills.{codex,gemini}.build_plan``) but not used here —
    Claude's target dirs are agent-owned (``~/.claude/skills/``), not oma-owned.
    Gemini stages its bundle under ``$OMA_HOME/extensions/gemini-oma/`` so it
