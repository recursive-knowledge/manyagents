"""The ``Adapter`` ABC — the whole contract (oma.adapters.md "The four-method
contract — Settled").
...
normalization/bounding/secret-scrub is ``oma.capture``'s job — keeping adapters
...
lifecycle stage of ``oma.capture.CanonicalTrace``. The adapter author emits
``CanonicalTrace``-shaped material (``oma.capture.md`` "Validate conformance");
``oma.capture`` validates → scrubs → bounds → persists. The ABC keeps the
frozen ``capture() -> RawTrace`` wording via this alias (see the
2026-05-19 Decision-log entry on ``oma.adapters.md``).
...
from oma.capture import CanonicalTrace
...
class Adapter(ABC):
    ...
    def capture(self) -> RawTrace:
        ...
        size-bounding, and persistence are NOT done here — ``oma.capture``
    ...
    def distill_model(self) -> object | None:
        """Optional: expose this agent's own model to ``oma.distill`` (the
        ``oma.utils.provider`` seam). Default: no model → caller falls back to
    ...
    def install_skills(
        ...
        *,
        ...
        ``oma.adapters.skills.<name>.install`` and return a
        :class:`oma._installer.Manifest` (or None if the user declined consent).
        Every filesystem write flows through ``oma._installer`` for transparency
        + ``oma uninstall <adapter>`` reversal."""
        return None


class PromptPrefixInjector:
    """The lowest-common-default ``inject``/``retrieve`` (oma.adapters.md
