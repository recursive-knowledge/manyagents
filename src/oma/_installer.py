"""oma._installer — the M11 transparency contract.

Adapters that install in-agent skills (`oma.adapters.skills.{claude,codex,gemini}`)
...
adapter:
  ...
  2. **Consent.** First run: print the plan and ask `[y/n/diff]`
     (``OMA_INSTALL_SKILLS=auto|prompt|deny`` overrides). Once recorded in the
  ...
  4. **Manifest.** Every write is logged at ``$OMA_HOME/installed/<adapter>.json``
...
# OMA_INSTALL_SKILLS=auto|prompt|deny. Default: prompt on first run, then auto.
...
def merge_toml_section(path: Path, section_path: str, value: dict[str, Any]) -> tuple[str, str | None]:
    ...
    ``mcp_servers.oma``). Comments and key order of the surrounding document
...
def _format_plan(plan: InstallPlan) -> str:
    lines = [
        f"oma → install in-agent skills for {plan.adapter!r} (scope={plan.scope}):",
        "",
    ]
    for op in plan.ops:
    ...
    lines.append("CREATE writes a new file (oma owns it). `oma uninstall <adapter>` reverses cleanly.")
    return "\n".join(lines)
...
def consent_prompt(
    ...
    *,
    ...
    """First-run consent gate. ``OMA_INSTALL_SKILLS`` env overrides:
    ``auto`` ⇒ silent yes; ``deny`` ⇒ silent no; ``prompt`` ⇒ always ask.
    Default: ask once (no manifest yet), then act like ``auto``."""
    mode = os.environ.get("OMA_INSTALL_SKILLS", "").strip().lower()
    if mode == "auto":
        return True
    if mode == "deny":
        output_fn("oma: OMA_INSTALL_SKILLS=deny — skipping skill install")
        return False
    if mode != "prompt" and manifest_exists:
        return True  # silent re-run after first consent
    if mode and mode not in _PROMPT_MODES:
        output_fn(f"oma: unknown OMA_INSTALL_SKILLS={mode!r}; treating as 'prompt'")
    ...
    while True:
        ...
        if ans in ("y", "yes"):
            return True
        if ans in ("n", "no", ""):
            output_fn("oma: install declined")
            return False
        if ans in ("d", "diff"):
...
def _run_cli(argv: list[str], *, description: str, stdin_input: str | None = None) -> None:
    ...
    try:
        ...
        print(f"oma: {description} — command failed ({type(exc).__name__}: {exc})")
        return
    if proc.returncode != 0:
        ...
        if tail:
            print(f"oma: {description} — exit {proc.returncode}: {tail[-1]}")
...
def uninstall(  # noqa: C901 — three reversal paths (CLI actions, create files, merge files: flat or nested) in sequence; refactoring would just shuffle the same complexity
    ...
    *,
    ...
    if manifest is None:
        output_fn(f"oma: no install manifest for {adapter!r} — nothing to do")
        return 1
    ...
    # Best-effort: prune empty install-root subdirs (`~/.claude/skills/oma-*`).
    for entry in manifest.entries:
        parent = Path(entry.path).parent
        with __import__("contextlib").suppress(OSError):
            if parent.is_dir() and not any(parent.iterdir()):
                shutil.rmtree(parent, ignore_errors=True)
    output_fn(f"oma: uninstalled {adapter} (manifest cleared)")
    return 0
