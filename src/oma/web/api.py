"""oma.web.api — the read-only HTTP surface over the Bank (M9).

This is ``oma``'s public attack/abuse surface (oma.web.md). Two invariants are
load-bearing and tested:

1. **Anon never receives a trace body, even with ``?include=raw``.** Raw bodies
   are outside the ``public`` role's grant at the database (oma.bank migration
...
(oma.core) — derivation (the agent activity span) lives in the frozen model,
pagination in ``oma.bank.make_cursor``. Identity is fixed at app construction
...
from oma.bank import Bank, get_bank, make_cursor
from oma.core import Agent, Packet
from oma.utils import config

# Identities permitted to read a raw trace body (oma.web.md "Trace/PII"):
...
def _clamp_limit(limit: int) -> int:
    if limit < 1:
        return 1
    return min(limit, config.OMA_WEB_MAX_PAGE_LIMIT)
...
def create_app(*, bank: Bank | None = None, identity: str = "public") -> FastAPI:  # noqa: C901 — N route closures inherently bump cyclomatic complexity
    ...
    app = FastAPI(
        title="Oh My Agent — read API",
        description="Read-only window over the Knowledge Bank (oma.web).",
        version="0.1.0",
    )
    app.state.bank = b
    ...
    async def session_view(
        ...
        limit: int = Query(default=config.OMA_WEB_PAGE_LIMIT),
        cursor: str | None = Query(default=None),
    ) -> Any:
        # `?p=` → one packet. The id is `{session}/{p}` — the exact URL
        # oma.distill emits (a curator bundle lives under the synthetic
    ...
    async def reuse_signal(
        ...
        # excluded — this is the "use as context" affordance (oma.web.md).
