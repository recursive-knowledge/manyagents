...
key (oma.bank.md "Access identities"). Network calls are retry-wrapped.
"""
...
from oma.bank.retry import with_backoff
from oma.utils import config

_IDENTITY_KEY_VARS: dict[str, str] = {
    "public": "OMA_BANK_ANON_KEY",
    "trusted": "OMA_BANK_TRUSTED_KEY",
    "admin": "OMA_BANK_ADMIN_KEY",
    "curator": "OMA_BANK_CURATOR_KEY",
}


def _cf_access_headers() -> dict[str, str]:
    cid = config.resolve("OMA_BANK_CF_ACCESS_CLIENT_ID", "")
    secret = config.resolve("OMA_BANK_CF_ACCESS_CLIENT_SECRET", "")
    if cid and secret:
...
class SupabaseBank:
    """Async :class:`~oma.bank.base.Bank` over Supabase PostgREST."""
    ...
    def __init__(self, identity: str = "trusted") -> None:
        ...
        self._url = config.resolve("OMA_BANK_URL", config.OMA_BANK_URL)
        # identity@url so oma.core's hydration cache never confuses two Banks
