...
Async because ``oma.core.Packet.fetch()`` is async (oma.core.md) and ``oma.web``
is FastAPI. The Bank trades plain ``dict`` records (the PostgREST row shape);
``oma.core`` (M3) layers the frozen Pydantic ``Packet`` on top — so the Bank
...
# Identity selects the access key (oma.bank.md 4-identity model).
...
class Bank(Protocol):
    ...
    # Stable per-Bank identity so oma.core's hydration cache never confuses two
