"""The mechanical post parser (oma.forum.md "Write-time discipline").
...
**hardened** for oma: swarms' ``evidence_post_id: int`` + ``task_id`` becomes
oma's ``evidence_ref``: a packet-id **string**, no task (the M6 analog of the
C3 swarms→oma ``Evidence`` mapping); plus oma-specific Bank-grounded checks
(``evidence_ref`` resolves, no-history hardening, quarantine refusal).

**C1 (oma.core.md:70/98; oma.forum.md:89):** :func:`parse_post` never
...
from oma.forum.schema import validate_schema
