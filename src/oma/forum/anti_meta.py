...
(oma.forum.md "Write-time discipline").
...
bundles were ~74% process meta-advice). ``oma.forum`` renders it into the
agent-side post prompt; ``oma.distill`` (M7) imports **this same object** so
...
block lives in the agent-side skill prompt `oma` injects, not in anything the
practitioner sees.
"""
...
# verbatim — mechanical substring, not model judgement (oma.forum.md:62).
...
# Abstract nouns that do NOT count as concrete grounding (oma.forum.md:62 /
...
# half, exactly as ``ANTI_META_BLOCK`` is for its prose half: ``oma.forum``'s
# post parser (M6) and ``oma.distill``'s bundle parser (M7) both import these,
...
# mechanical — never trusted to the model (oma.forum.md:62 / oma.distill.md:57).
# --------------------------------------------------------------------------- #
...
def has_banned_meta(text: str) -> str | None:
    ...
    model judgement (oma.forum.md:62)."""
    low = text.lower()
