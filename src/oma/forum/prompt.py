"""The agent-side post prompt renderer (oma.forum.md "Write-time discipline":
*rendered into the agent-side post prompt*).

This lives in ``oma.forum`` on purpose: the module that owns the parser also
...
No-history hardening (oma.forum.md "Forge protection"): when a ``goal`` has no
...
from oma.forum.anti_meta import ANTI_META_BLOCK
