...
Explicit async ``.fetch()`` hydration (cache → oma.bank). A bare ``Packet(...)``
...
class Goal(BaseModel):
    ...
    oracle. Never gates anything (oma.core.md)."""
...
class Agent(BaseModel):
    ...
    # Derived activity span for the oma.web ``GET /s/{session}/agents`` route
    # (oma.web.md). Defaulted None; populated only by :meth:`from_activity`.
    ...
    def from_activity(cls, row: dict[str, Any], *, packets: Iterable[dict[str, Any]] = ()) -> Agent:
        ...
        ``oma.web`` route fetches the agent + packet rows and hands them here,
        so the route stays a dumb orchestrator and the derivation lives in the
        frozen model (oma.web.md; Design Principles §3, §4).
...
class _PacketFields(BaseModel):
    ...
    def _check_shape(self) -> _PacketFields:
        ...
        if self.preference is not None and self.type != "distill":
            # C1 (oma.core.md:70/98): preference=accept|reject is distill-only,
            ...
            # never carry preference. Mechanical, past the oma.forum parser.
            raise ValueError("preference is distill-only (oma.core.md; C1) — a post never carries it")
        return self


class KnowledgePacket(_PacketFields):
    """The canonical public wire shape (oma.web). Mirrors Packet's public
