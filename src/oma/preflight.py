...
(M8; oma.cli.md "preflight.py").

Three checks, fail-fast with a specific message:
  1. env presence — ``OMA_BANK_URL`` and at least one Bank key.
  ...
  3. migration inventory — the ``supabase/migrations/*.sql`` files oma.bank
...
_BANK_KEYS = ("OMA_BANK_ANON_KEY", "OMA_BANK_TRUSTED_KEY", "OMA_BANK_ADMIN_KEY", "OMA_BANK_CURATOR_KEY")
_MIGRATIONS_DIR = Path(__file__).resolve().parents[2] / "supabase" / "migrations"


def _check_env() -> str | None:
    if not os.environ.get("OMA_BANK_URL"):
        return "OMA_BANK_URL is unset (point it at the Supabase PostgREST URL)"
...
def _live_schema_diff(url: str) -> str:
    ...
    anon grant, so without ``OMA_BANK_ADMIN_KEY`` this is skipped, not failed —
    the offline / non-privileged path is byte-for-byte unchanged."""
    admin = os.environ.get("OMA_BANK_ADMIN_KEY")
    if not admin:
        return "skipped (no OMA_BANK_ADMIN_KEY; schema_migrations needs the privileged role)"
...
def run_preflight() -> int:
    ...
    reason = _check_bank_reachable(os.environ["OMA_BANK_URL"])
    if reason is not None:
    ...
    print(f"[INFO] schema diff: {_live_schema_diff(os.environ['OMA_BANK_URL'])}")
    return 0
