"""The single ``oma`` console-script entrypoint (M8; oma.cli.md).

A **dumb orchestrator**: it sequences ``oma.core`` + ``oma.adapters`` +
``oma.capture`` + ``oma.forum`` + ``oma.distill`` + ``oma.bank`` and owns no
...
**C1 (oma.core.md:70/98; oma.forum.md:89):** a rejected ``/self-distill`` post
is **not persisted** — the agent is re-prompted. ``preference=accept|reject``
is distill-only (``/cross-distill``). This supersedes ``oma.cli.md:61``'s
...
from oma import __version__
from oma.bank import Bank, get_bank
from oma.utils import config, sid
...
# (oma._mcp + oma.adapters.skills.*). M11.4 ripped the bash surface;
# scripts/programmatic callers use ``oma._handlers.do_*`` directly.
...
def _oma_home() -> Path:
    """``~/.oma`` (or ``OMA_HOME`` — tests point this at a tmp dir so the real
    home is never touched)."""
    return Path(os.environ.get("OMA_HOME", str(Path.home() / ".oma")))


def active_session_path() -> Path:
    return _oma_home() / "active"
...
def _resolve_sid(explicit: str | None) -> str:
    """``--session`` wins, else ``~/.oma/active``; error if neither."""
    s = explicit or _read_active()
    if not s:
        raise SystemExit("no active session: run `oma start` or pass --session <id>")
    return s
...
def ask_rating(propose: int | None, *, input_fn: In, output_fn: Out, noninteractive: bool) -> int | None:
    """The ★ prompt. ``OMA_NONINTERACTIVE`` ⇒ unrated (no prompt). Else the
...
def ask_yn(prompt: str, *, input_fn: In, output_fn: Out, noninteractive: bool) -> bool:
    """A ``[y/n]`` gate. ``OMA_NONINTERACTIVE`` ⇒ deny-by-default (Open-Q §B5):
    no inject, no destructive confirm without a human present."""
    if noninteractive:
        output_fn(f"  (OMA_NONINTERACTIVE: '{prompt}' → denied)")
        return False
    return input_fn(f"{prompt} [y/n]: ").strip().lower() in ("y", "yes")


# --------------------------------------------------------------------------- #
# two-stage SIGINT (datasmith precedent: oma wraps a live child agent)
# --------------------------------------------------------------------------- #
...
def _sigint_handler(signum: int, frame: object) -> None:
    ...
    _sigint_count += 1
    from oma.adapters import terminate_all_agents

    if _sigint_count >= 2:
        print("oma: force-killing agent subprocesses", file=sys.stderr)
        terminate_all_agents(force=True)
        os._exit(1)
    print("oma: interrupted — terminating agents (Ctrl-C again to force-quit)", file=sys.stderr)
...
# Adapter / agent / headless-model helpers moved to ``oma._handlers``.
# --------------------------------------------------------------------------- #
...
def _session_url(session_id: str) -> str:
    ...
    host = config.resolve("OMA_WEB_HOST", config.OMA_WEB_HOST) or "127.0.0.1"
    if host == "0.0.0.0":  # noqa: S104 — defensive REWRITE of an unreachable wildcard, not a bind
        host = "127.0.0.1"
    port = int(config.resolve("OMA_WEB_PORT", config.OMA_WEB_PORT, cast=int))
...
def _agent_url(agent_id: str) -> str:
    ...
    matching the ``oma.web`` route convention (full id = ``{session}/{tail}``)."""
...
async def _do_register(args: argparse.Namespace, *, bank: Bank, io: tuple[In, Out]) -> int:
    from oma._handlers import _resolve_agent
...
async def _do_run_agent(
    ...
    from oma._handlers import _adapter_for, _resolve_agent
    ...
    try:
        ...
        io[1](f"oma: skill install skipped ({type(exc).__name__}: {exc}) — see `oma status`")

    # Thread OMA_SESSION into the child so the MCP server (spawned by the
    # agent) can resolve the active session even without ~/.oma/active.
    os.environ["OMA_SESSION"] = sid_

    argv = [adapter.binary, *agent_args]
    io[1](f"oma: running {' '.join(argv)} (session {sid_})")
    ...
    tee_fd, tee_name = tempfile.mkstemp(prefix=f"oma-tee-{sid_}-", suffix=".log")
    os.close(tee_fd)
    ...
    try:
        ...
        io[1](f"oma: captured raw packet {pid} ({len(tee_bytes):,} bytes)")
    except Exception as exc:
        io[1](f"oma: trace capture failed ({type(exc).__name__}: {exc})")
    return 0
...
async def _do_uninstall(args: argparse.Namespace, *, bank: Bank, io: tuple[In, Out]) -> int:
    ...
    rc = uninstall(args.adapter, _oma_home(), output_fn=io[1])
    if rc == 0:
        io[1]("")
        io[1](
            f"oma: skill files are gone from disk; if {args.adapter} is currently "
...
async def _do_status(args: argparse.Namespace, *, bank: Bank, io: tuple[In, Out]) -> int:
    """``oma status`` — list every adapter that currently has an in-agent
    install (skills + MCP server entry) along with the files it owns."""
    from oma._installer import list_installed

    manifests = list_installed(_oma_home())
    if not manifests:
        io[1]("oma: no in-agent skills installed (run `oma <adapter>` to install)")
        return 0
    for m in manifests:
...
def _pty_spawn(argv: list[str], *, tee: Path | None = None) -> None:  # noqa: C901 — a PTY bridge is irreducibly stateful
    ...
    Monkeypatched in unit tests; the real path is exercised by ``oma <name>``.
    ...
    **POSIX only.** Windows has no ``pty``/``fcntl``/``termios``/``tty``; on
    ...
    if sys.platform == "win32":  # pragma: no cover — checked on Windows CI
        raise NotImplementedError(
            "oma's PTY wrapper is POSIX-only (Windows lacks pty/fcntl/termios). "
            "Run the wrapped agent directly (e.g. `claude`); /self-distill etc. "
            "still work via the MCP server once `oma <name>` installed the skills."
        )
...
# M11.4: the four knowledge-loop verbs moved to ``oma._handlers``. They are
...
# The user-facing path is in-agent skills + the MCP server (oma._mcp).


async def _do_end(args: argparse.Namespace, *, bank: Bank, io: tuple[In, Out]) -> int:
    sid_ = _resolve_sid(args.session)
    await bank.put_session(sid_, status="ended")
    # ★ moment: oma.core has no sessions.rating column, so oma end's ★ lands on
    # the most recent UNRATED reflection post in the session (no-op if none) —
    # avoids an unneeded oma.bank migration (M8 Decision-log).
...
def _noninteractive() -> bool:
    return config.resolve("OMA_NONINTERACTIVE", False, cast=config.as_bool)
...
session lifecycle (the only verbs on this CLI):
  ...
  oma register <name>             register an adapter as an Agent
  oma <name> [args...]            install in-agent skills + run the wrapped agent under a PTY
  oma end [--session id]          end the session (optional ★ prompt)
  oma status                      list installed in-agent skill bundles
  oma uninstall <adapter>         reverse the install via the saved manifest
...
Skills + MCP server install on `oma <name>`; the human surface stays one tap
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--version", action="version", version=f"oma {__version__}")
    ...
    # ``oma <name>``. Programmatic callers use ``oma._handlers.do_*`` directly.

    e = sub.add_parser("end")
    e.add_argument("--session")

    # M11: in-agent install lifecycle. `oma <name>` runs the install
...
def _guard(coro: Coroutine[Any, Any, int]) -> int:
    ...
    is what ``python -m oma.preflight`` diagnoses; here we just fail cleanly and
    point at it. ``OMA_DEBUG=1`` re-raises for developers."""
    try:
        return asyncio.run(coro)
    except KeyboardInterrupt:
        print("oma: interrupted", file=sys.stderr)
        return 130
        ...
        if isinstance(exc.code, int) or exc.code is None:
            raise
        print(f"oma: {exc.code}", file=sys.stderr)
        return 1
    except Exception as exc:
        if config.resolve("OMA_DEBUG", False, cast=config.as_bool):
            raise
        print(f"oma: {type(exc).__name__}: {exc}", file=sys.stderr)
        print(
            "oma: run `python -m oma.preflight` to check env/Bank/keys — set "
            "OMA_BANK_TRUSTED_KEY in oma.env, or start a local Bank with "
            "`make bank-up`. Set OMA_DEBUG=1 for a full traceback.",
            file=sys.stderr,
        )
        return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    # Not a known verb and not a flag → `oma <name> [args]` (run an agent).
