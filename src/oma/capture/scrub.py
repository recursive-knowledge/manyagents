"""Secret scrub — *defense-in-depth* (oma.capture.md "Secret scrubbing").

The primary control is the access boundary: ``oma.bank`` migration ``00004``
...
version (not an ``OMA_*`` runtime knob): a re-scrub backfill compares each
``traces.scrub_version`` row against this constant and retro-quarantines newly
detected leaks (oma.capture.md "Operations & recovery").

Invariant: a :class:`~oma.capture.models.ScrubReport` carries **counts only,
