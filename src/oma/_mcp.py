"""oma._mcp — the in-agent MCP server (M11).

One Python MCP server exposing oma's knowledge-loop verbs as tools any
...
gate — no string-parsing of y/n inside the chat (oma.web.md / advisor).

Run: ``python -m oma._mcp`` (the per-adapter installer registers this).
"""
...
from oma import setup_environment

setup_environment()  # ~/.oma/env then ./oma.env — Bank creds for the MCP child

app = FastMCP("oma")


# --------------------------------------------------------------------------- #
# session id resolution: OMA_SESSION env wins, else ~/.oma/active.
# This dual-source lets `oma <name>` thread the session at PTY-spawn time AND
# lets the agent (opened later/anywhere after one-time `oma start`) still work.
# --------------------------------------------------------------------------- #


def _session_id() -> str:
    sid = os.environ.get("OMA_SESSION", "").strip()
    if sid:
        return sid
    home = Path(os.environ.get("OMA_HOME", str(Path.home() / ".oma")))
    p = home / "active"
    if p.is_file():
        ...
        if s:
            return s
    raise RuntimeError(
        "no active oma session — run `oma start` first (writes ~/.oma/active), "
        "or export OMA_SESSION=<id> for this MCP server"
    )
...
async def self_distill_draft(guidance: str = "") -> dict[str, Any]:
    ...
    invoked (C1: oma.cli.md:48; oma.core.md:147)."""
...
async def discuss_draft(stance: str, packet: str | None = None) -> dict[str, Any]:
    ...
    retrieval-before-post (the mechanical gate `oma.forum` enforces). Returns
    ...
    from oma.bank import get_bank
    from oma.forum import retrieve
    from oma.forum.prompt import render_post_prompt
    ...
    if not ranked:
        return {
            "error": "no related posts to engage — call self_distill_draft first (retrieval-before-post; oma.forum)",
        }
...
async def commit_post(
    ...
    from oma.utils import sid as sid_
...
if __name__ == "__main__":  # pragma: no cover — `python -m oma._mcp`
    main()
