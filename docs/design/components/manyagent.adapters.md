...
- **2026-06-23 — Codex skill invocation prefix dropped: `$manyagent-<verb>` → `$<verb>` (skill dirs `~/.codex/skills/<verb>/`).** Per explicit user direction, the four Codex skills are now invoked as `$self-distill` / `$discuss` / `$cross-distill` / `$inject` (was `$manyagent-self-distill` etc.), and their on-disk directories are the bare verbs (`~/.codex/skills/self-distill/SKILL.md`, with YAML `name: self-distill`) rather than `manyagent-<verb>`. This **supersedes** the M11 build decision (`manyagent.cli.md` 2026-05-20 entry, and the original `manyagent.adapters.skills.codex` docstring rationale) that prefixed the package name to avoid a collision in Codex's `$` namespace: we now accept that collision risk in exchange for the shorter, host-symmetric surface (Claude/Gemini already type the bare `/<verb>`). Scope was deliberately narrow — **unchanged**: the MCP server name `manyagent` (`[mcp_servers.manyagent]`), the dotted MCP tool refs (`manyagent.commit_post` / `manyagent.inject_commit`), and Claude/Gemini skills (already bare `/<verb>`). Changed: `manyagent.adapters.skills.codex` (`_DIALECT.invocation` lambda → `f"${slug}"`, the SKILL.md write path / YAML `name:` / `description` / `commands` table / module docstring), plus stale-descriptive prose in `manyagent.adapters.builtin.codex`, `manyagent._skills`, `manyagent.adapters.skills.__init__`, and `docs/guide/authoring-an-installer.md`'s current-state table. Tests in `tests/test_skills.py` and `tests/test_adapter_install.py` updated to assert the bare-verb dirs/invocations (the renamed `test_codex_plan_creates_bare_verb_skills_and_toml_merges`). **README needed no change** — it contains no `$manyagent-` reference (and, contrary to the task brief's assumption, no `$self-distill` literal either; the slash verbs are described, not invocation-spelled). The dated M11 entry in `manyagent.cli.md` was left untouched (historical record); its `$manyagent-<verb>` text is superseded by this entry, not edited in place.
