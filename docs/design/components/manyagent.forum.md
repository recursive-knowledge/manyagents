...
- **2026-08-09 — `ANTI_META_BLOCK` stops instructing the curator to fill a field manyagent does not have.** The block (ported verbatim from swarms) said *"Put supporting forum post IDs in `evidence_post_ids`… if the only support is an attempt, leave `evidence_post_ids` empty"*. manyagent has no such field: grounding lives in each Insight's `evidence: [{post_id, quote}]` and `manyagent.distill.parse.validate_bundle` reads nothing else. The same system prompt therefore described **two** grounding mechanisms, and an Insight grounded only through the dead one arrives with an empty `evidence` list and is dropped by the parser — a silent loss of exactly the Insights the model believed it had grounded. The 2026-06-11 entry already named `evidence_post_ids` as a foreign referent that leaked into a reflection; this removes it at the source. The clause now names the real field and restates the verbatim-quote requirement the parser enforces. `_CURATOR_ONLY_PHRASES` moved its anchor from `evidence_post_ids` to `PREFER transferable wording` (a clause that genuinely exists only in the curator block), so `assert_anti_meta_rules_present` keeps its leak guard. Both blocks and the post schema were also rewritten to ASD-STE100 discipline — one instruction per sentence, active voice, simple tenses, numbered lists for the parser's drop conditions — because the same text is read by an agent at write time and by a human auditing the corpus. `BANNED_META_PHRASES` and the mechanical primitives are untouched, so the single-source contract with the parser is unchanged. Tests: `tests/test_forum.py::test_post_anti_meta_block_shares_blacklist_without_curator_referents`, `::test_assert_anti_meta_rules_present_post_prompt_mode`.
- **2026-08-09 — `CONCRETE_RE` learns the terminal domain.** Its alternatives (backticked token, `dotted.path`, `call()`, `--flag`, `/path`, snake_case, CamelCase, `Error`/`Exception`) came from swarms, whose benchmarks were ARC, SWE-bench and polyglot — grid operations and code identifiers. KSI (arXiv:2607.19592) also evaluates a **terminal** family, where the concrete primitives are log markers and command output. Measured on the terminal scenario of `scripts/simulate_ksi.py`: the Insight *"nginx accepts duplicate listen directives in different files, causing `[emerg]` errors"* was dropped as non-concrete while a sibling Insight built from the **same post** survived, purely because the model happened to backtick `` `nginx -t` `` there and not here — so retention depended on the model's incidental formatting rather than on the claim's substance. Added one alternative, `\[[a-z]{3,}\]`, matching a lowercase word in square brackets (log-level shaped). It stays deliberately narrow because every widening of this gate admits prose into a public corpus: a one- or two-letter bracket (array index, footnote) still fails, and prose/abstract nouns still fail. Tests: `tests/test_forum.py::test_is_concrete_accepts_bracketed_severity_marker`, `::test_is_concrete_still_rejects_prose_and_abstract_nouns`.
- **2026-08-09 — the post prompt now asks for `backticks`, because the parser has always required code-shaped text.** Measured with `scripts/probe_post_gate.py` (8 live `/self-distill` posts from `qwen3.6-35b-a3b`, real `render_post_prompt` in and real `parse_post` out): **4 of 8 posts were rejected, and 3 of those 4 failed on `load_bearing_assumption is not concrete`** — for claims like *"systemctl restart nginx returns exit code 0 on invalid configuration files"*, which names a literal command and a literal exit code. `CONCRETE_RE` matches code-*shaped* material (a backticked token, `dotted.path`, `call()`, `--flag`, `/path`, snake_case, CamelCase, an error type), and a bare command sitting in prose matches none of them. Every **stored** post in that run happened to backtick its command, so whether a lesson entered the corpus turned on the model's incidental formatting rather than on the claim's substance — the same pattern found on the curator side the same day. The gate is not wrong; the instruction underspecified it. The prompt said "concrete — names a specific tool/API/file", which a model satisfies in prose while failing the regex: an instruction/enforcement mismatch of the same class as the `evidence_post_ids` clause. Note `manyagent.web.skill`'s SKILL.md already told MCP agents to use "a backticked identifier", so the MCP frontend was getting a stronger instruction than the CLI and headless paths. `POST_ANTI_META_BLOCK` and the post `_SCHEMA` now name shell commands as primitives and ask for backticks explicitly, with a worked contrast (`` `systemctl restart nginx` returns 0 `` versus the bare form that is dropped). Re-measured over 8 fresh posts: **`is_concrete` rejections 3 → 0**, every remaining rejected assumption reporting `is_concrete=True`, stored 4/8 → 5/8. The remaining rejections have different causes (paraphrased rather than verbatim `evidence`, one missing `proposed_next`) and are not addressed here. n=8 is too small for a precise rate; the mechanism change is what the numbers establish.
