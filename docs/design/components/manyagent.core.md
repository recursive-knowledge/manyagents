...
`manyagent.core` is the model layer. Pydantic v2 frozen value objects; explicit async `.fetch()` hydration (no per-instance `__getattr__` — Design Principles §4). The 2026-05-19 changes add `Goal`, the swarms-aligned packet taxonomy, and the human/behavioral outcome fields.
...
class Packet(BaseModel):
    ...
    structured: dict | None = None  # falsifiable post-mortem schema (manyagent.forum)
    ...
    bundle: dict | None = None      # 6 typed Insight buckets (manyagent.distill)
    ...
    async def fetch(cls, id: str) -> "Packet":
        """Hydrate from manyagent.bank (cache → API). Explicit; the Overview REPL
        elides the await. Bare `manyagent.Packet(id)` does no I/O."""
