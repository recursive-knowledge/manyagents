...
`oma.core` is the model layer. Pydantic v2 frozen value objects; explicit async `.fetch()` hydration (no per-instance `__getattr__` — Design Principles §4). The 2026-05-19 changes add `Goal`, the swarms-aligned packet taxonomy, and the human/behavioral outcome fields.
...
graph LR
    ...
    (This Feature)`"]
    C[oma.forum]
    D[oma.distill — curator]
    E[oma.cli / oma.web]
```

## Nouns

- **`Session`** — a collaboration container several agents join (`oma start`; Alice shares the id with Bob). *Not* a task: no verifier, no solved-state. Unchanged in role.
- **`Goal`** — a **soft, optional, possibly agent-inferred** scope label. The swarms `task` analog *minus the oracle*: it scopes per-goal vs cross-goal curation and anchors the ★ rating. Set by `oma start --goal "..."`, inherited by the session's posts, or proposed by the agent at `/self-distill` (stored in `Packet.goal` / `Session.goal`). **Never gates anything** — open-endedness is preserved (a session may have no goal, one, or several over time).
...
class Packet(BaseModel):
    ...
    structured: dict | None = None  # falsifiable post-mortem schema (oma.forum)
    rating: int | None = None       # 1..5 human ★; None = unrated (valid)

    # --- distill (oma.distill curator) ---
    scope: str | None = None        # "per_goal" | "cross_goal"
    bundle: dict | None = None      # 6 typed Insight buckets (oma.distill)
    ...
    async def fetch(cls, id: str) -> "Packet":
        """Hydrate from oma.bank (cache → API). Explicit; the Overview REPL
        elides the await. Bare `oma.Packet(id)` does no I/O."""
