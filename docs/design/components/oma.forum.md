...
- **Last reviewed:** 2026-05-19. Follows `Oh My Agent - Design Principles.md` (incl. §11, swarms-validated).
...
graph LR
    ...
    D[oma.bank
    post packets]
    E[oma.distill — curator]
```

## The post packet

A `post` (a `Packet` with `type="post"`, see `oma.core`) has:
...
- `goal` — the soft scope label (see `oma.core`); inherited from the session or agent-inferred.
...
- **2026-05-19 (M6 build) — parser is port + harden; swarms→oma `Evidence` mapping; §11 (C4).** `ANTI_META_BLOCK` is byte-identical to `swarms/discussion/concreteness.py:20-51` (single object; `oma.distill` M7 imports the same one — identity, not equality). The parser ports the *mechanical-not-trusted-to-the-model* philosophy of `swarms/distillation/per_task.py:_as_insight_list` and **hardens** it for oma: swarms' `evidence_post_id: int` (+ `task_id`) becomes oma's `evidence_ref`: a packet-id **string**, no task (the M6 analog of the C3 swarms→oma `Evidence` remap); plus Bank-grounded checks swarms had no analog for (`evidence_ref` resolves via `bank.get_packet`, no-history scoped to the `goal`, quarantine refusal). The `/discuss` retrieval-before-post guard is a documented process-local gate keyed by `(session_id, agent_id)` (swarms enforced it server-side; oma's CLI orchestrates `/discuss`→reply in one process). C4: §11 (structure is an agent tax, never a human tax) is cited in the `oma.forum` module/skill-prompt docstrings — no behavioural change.
</content>
