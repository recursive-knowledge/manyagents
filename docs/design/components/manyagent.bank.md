...
- **2026-06-14 (00011) — `agents.principal_id`: persistent cross-goal agent identity.** New nullable column (migration `00011_agent_principal.sql`, `add column if not exists` + `agents_principal_idx`) stamped with the operator's stable per-(machine, adapter) principal — a UUID4 minted client-side (`manyagent.cli._principal_for`, persisted in `~/.manyagent/principals.json`) so one operator's "claude" is the SAME agent across every goal/session. The agent **id scheme is unchanged** (principal is a column, not part of the id) so packet ids / FK topology / the 00010 prefix rewrite are untouched. New Bank method `list_agents_by_principal(principal_id)` (real + FakeBank parity); `put_agent` gained a defaulted `principal_id` kwarg (legacy callers source-compatible). **No new RLS** — the `agents` grants are table-scoped and `public_read` is `USING (true)` (00004), so a new column inherits the table policies; legacy rows stay NULL and the viewer degrades to the session-scoped agent view.
