...
A random **UUID4** in canonical form (lowercase, hyphenated, 36 chars). `new()` still does the optional Bank existence check (collision-safe). `parse()` normalizes lenient user input (case, whitespace, the no-hyphen/brace forms) to canonical; `is_valid()` validates the canonical form (used by `manyagent.core.Session`). UUIDs are URL-safe and contain no `/`, so the `{session_id}/{suffix}` packet-id scheme stays unambiguous. datasmith identity rule applies: this is a real key, never a derived string. (Human-facing URLs use a goal slug, not the id — see `manyagent.web.viewer`.) Superseded the Crockford-Base32 `XXXX-XXXX` codec on 2026-06-13 (Decision log).
...
* **Unit:** `sid.new()` 10k unique + valid (canonical UUID4); `parse()` normalizes case/whitespace/no-hyphen forms; `is_valid()` rejects non-canonical (uppercase, braced, hyphenless) and non-UUID input. Config precedence per layer. `provider.resolve()`: adapter hook → wrap; fallback configured → build; neither → documented error. `rate_limit_signal()` parses canned Codex/Claude rate-limit payloads into a correct retry-after. `log` emits the exact `[DEBUG]`/`[INFO]` prefixes.
...
- **2026-06-13 — Session-id codec switched from Crockford Base32 (`XXXX-XXXX`) to UUID4 (user decision).** The short human-typable id wasn't needed; a plain UUID is the cleaner identity (and keeps the door open to unifying with a harness session id later). `sid.py` now generates `uuid.uuid4()` in canonical form; `is_valid`/`parse` validate/normalize UUIDs (the Crockford alphabet/`format()` are gone). Storage was already format-agnostic (session id is opaque `text`; `manyagent.capture.conformance` only forbids `/`, which UUIDs honor), so no schema change was required for new sessions — but existing Crockford rows were **backfilled** to UUIDs by `supabase/migrations/00010_uuid_session_ids.sql` (one transaction; remaps every FK column AND the composite `{sid}/…` ids AND the un-constrained embedded-id columns `packets.{agent_id,reply_to,parent_attempt,parents[]}`). Human-facing website URLs now use an ≤80-char goal slug instead of the id (`manyagent.web.viewer`). The packet-suffix generator in `manyagent._handlers`/`manyagent._skills` was decoupled from the session codec to `uuid.uuid4().hex[:8]` (matching `manyagent.capture`).
