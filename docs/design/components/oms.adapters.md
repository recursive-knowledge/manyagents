...
- **2026-06-10 — headless `distill_model()` hygiene: no `OMS_SESSION` inheritance; claude shells `--output-format json` with an envelope unwrap.** Two fixes from the FA04-ESNF trial. (1) `_HeadlessModel.complete` spawned the distiller CLI with the wrapper's full environment — including the `OMS_SESSION` that `_do_run_agent` exports for the PTY child. The user-scope `oms._hook` inside the spawned CLI therefore treated the *distiller* as a wrapped run: it appended the distiller's own harness session id + transcript path to `$OMS_HOME/bindings/<sid>.jsonl` (poisoning what `Adapter.mine()`/trace-grounding later reads — the wrong-session contamination) and delivered the session's inject stash as the distiller's SessionStart `additionalContext`. The shell-out now strips `OMS_SESSION` from the child env (everything else preserved). (2) `ClaudeAdapter._distill_cmd_prefix` is now `["claude", "-p", "--output-format", "json"]`: the headless answer arrives as one machine-readable `{"type": "result", "result": "<text>", …}` envelope instead of free prose at the mercy of markdown fences. `_HeadlessModel` gained an optional `extract` hook (default identity, supplied per adapter via the new `_StructuredBuiltin._distill_extract`); claude's override returns the envelope's `result` text (empty string on an error envelope — the caller's "no parseable post" path, never envelope-as-post garbage) and passes non-envelope output through untouched, so codex/gemini and the `OMS_LLM_*` fallback are unaffected. Tests: `tests/test_adapters.py` (env strip, prefix, unwrap matrix, extract-applied-through-`distill_model()`).
- **2026-06-10 (M13.1) — `Adapter.mine(ctx)` (optional sixth hook) + `oms.adapters.miners.claude`.** The harness-rendition seam from the Trace Renditions & Mining design doc §4a. ``MineContext = {cwd, window, bindings}`` (frozen dataclass in ``adapters.base``); the default returns None (no miner). The Claude miner reads every transcript named by this run's ``oms._hook`` binding records (``binding: "hook"`` — survives ``/clear`` rolling new session files; verified on the live corpus: a single run yielded 2–3 segments), falls back to scanning ``~/.claude/projects/<munged-cwd>/`` by run-window mtime (``binding: "scan"`` — recovered the pre-hook trial trace), and parses per-line defensively (``user`` string/block content; ``assistant`` ``text`` blocks → assistant turns, ``tool_use`` blocks → tool turns with a 400-char input preview; metadata entry types skipped; torn lines never kill a segment). Every text field is scrubbed **per-string** before the artifact leaves the miner — scrubbing the serialized JSON instead could let an env-kv redaction span structure and corrupt the document. Caps: 4000 chars/turn, 2000 turns/segment; ``completeness: full|partial`` is honest about unreadable bound files. ``DummyAdapter`` (oms.testing) gained ABC parity (``mine`` → None).
- **2026-06-10 (M13.1 hardening, adversarial review) — scrub-before-cap, basename-only path, tight scan window, size bound.** Review of the miner confirmed four issues, all fixed in `oms.adapters.miners.claude`: (1) **the on-disk `transcript_path` leaked the OS username + home tree into the public rendition** (it was never scrubbed and `oms.capture.scrub` has no path rule) — the segment now carries only the file basename (`transcript`), and the viewer never displayed the full path anyway. (2) **Cap-before-scrub** stranded sub-floor credential fragments: `_parse_transcript` now stores UNCAPPED text and `_scrub_then_cap` scrubs the full strings *then* truncates, so a key spanning the cap is redacted before truncation can hide its tail (the capture pipeline's documented scrub-before-bound order, which the miner had inverted). (3) **Scan-tier cross-contamination**: `_WINDOW_SLACK_S` cut 120s→15s — mining runs at the child's exit, so a transcript written well after belongs to a *different* session; a wide window swept concurrent same-cwd sessions into one run's public rendition. The hook tier (exact bound paths) is immune and remains the default. (4) **No size bound** vs the raw path's `OMS_TRACE_MAX_BYTES`: `_bound_artifact` caps the serialized body at 1 MB, dropping trailing turns/segments and downgrading `completeness`. In-conversation home paths in turn TEXT remain (same content the raw trace already exposes; scrub v1 quasi-identifier scope, unchanged). Tests added for each: basename-only, boundary-straddling key redaction, size bound.
- **2026-06-11 — headless `distill_model()` runs from an empty temp cwd (hermetic distiller).** Completes the 2026-06-10 hygiene entry's third leg. A live distillation (session `3BK6-0AFF`) showed the spawned `claude -p` — running in the repo cwd — loading its OWN project context (CLAUDE.md, git status) and blending it into the reflection as if it were session evidence (`proposed_next` cited "the primary design doc listed in git status"; the session trace contained no such resolution). `_HeadlessModel.complete` now runs the shell-out inside a `tempfile.TemporaryDirectory` (`cwd=` the empty dir, removed after): the spawned CLI finds no CLAUDE.md, no `.git`, no project dir, so the trace rendered into the prompt (oms.forum.md same date — which also gained the prompt-side fence sentence) is the distiller's only knowledge of the session. Applies to all three builtins via `_StructuredBuiltin.distill_model()`. Test: `tests/test_adapters.py::test_headless_complete_runs_in_hermetic_cwd`.
