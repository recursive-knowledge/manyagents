...
7. **Cross-distill concurrency.** Resolved simple: Supabase is ACID; two simultaneous `/cross-distill` calls just produce two `cross-distill` packets; consumers take the **latest**. No locking, no coordination. (`oms.distill`/`oms.bank`.)

8. **Plugin ecosystem / "Bank API versioning".** There is no arbitrary ecosystem. An adapter is a **pull request to the GitHub repo**, manually reviewed by a maintainer, open-source, merged only if non-nefarious. The hub serves reviewed/merged plugins. No untrusted-code-execution trust model, no multi-client schema-compat problem. (`oms.adapters`.)

9. **PTY / capture fidelity.** Not an `oms.capture` heuristic problem. The `CanonicalTrace` OO schema is the contract; the *adapter author* is responsible for emitting schema-conformant traces and is reviewed at PR time. `oms.capture` validates conformance, bounds size, scrubs, persists — it does not magically parse arbitrary terminals. (`oms.capture`/`oms.adapters`.)

10. **Identity / abuse / "no-auth".** Resolved via the 3-role model (datasmith-validated): `public` (read-only), `trusted` (writes with a manually-distributed key), `admin` (full oversight), enforced by Supabase-native RLS + role grants, exposed via PostgREST. Not "no auth"; a deliberate minimal-auth wall-back adopted from the start. (`oms.bank`/`oms.web`.)

11. **Injection trust boundary.** Resolved via human-in-the-loop: `/inject` shows a preview (first 100 + last 100 tokens of the packet summary) and requires explicit y/n; deny aborts. No reputation/trust-graph needed for v1. (`oms.cli`/`oms.distill`.)

## D. The one genuinely-hard thing still partly open

12. **"What is a good distillation?" — design RESOLVED; one empirical question now tracked.** The *definition* is no longer open: it is the swarms discipline, adopted in `oms.forum`/`oms.distill` (concrete+bounded+grounded+scarce, enforced write+curate-time, mechanically validated). What remains is **empirical, and it is the new headline experiment** (see §A1-NEW below), not a design blocker.

## A-NEW. Tracked experiments (design settled; needs running data to confirm)

13. **Validate downstream-reuse as the curation weight (the new headline question).** Per the 2026-05-19 decision, `reuse_score` (a packet was `/inject`ed into a later session that was then rated/accepted well) is the **default baseline** curation weight — a behavioral substitute for swarms' objective `native_score`/recurrence. It is *designed and implemented* (`oms.bank` `injections` ledger + recomputable view; `oms.distill` weighting). **Open empirically:** does reuse-weighting actually improve injected-bundle usefulness vs. ★-only or unweighted? This needs real corpus data. Tracked as the primary `oms` experiment once there is a running deployment. Owner: us, post-deployment.
