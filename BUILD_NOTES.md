...
Durable cross-milestone tracking for the `oma` build. Survives context resets;
...
- **C1 — M6/M8: `/self-distill` rejection.** A rejected `/self-distill` post is
  ...
  **distill-only** field (`oma.core.md:70`), set via `/cross-distill`
  ...
  reject⇒re-prompt not stored**. `oma.cli.md:61` ("stores preference=reject")
  superseded — dated C1 entry on both `oma.cli.md` copies.
- **C2 — M10 Risks: `poison_check`.** The one intentional Fragile (Design
  ...
  `oma.distill`, behind `/inject` human gate + quarantine + no-carry-forward.
  v1 heuristic optional. Status: implement seam in M7, document risk in M10.
- **M10 env-doc TODO (collected).** New `OMA_`-prefixed knobs introduced after
  M1's `oma.env.example` need an `oma.env.example` line at M10 (Design
  Principles §8): `OMA_ADAPTERS_DIR` (M5, local-adapter root) and `OMA_HOME`
  (M8, `~/.oma` active-session dir override). Code-defaulted + greppable
  already; M10 just documents them in `oma.env.example`.
- **C3 — M7: port + harden, not verbatim.** swarms enforces
  ...
  mechanical `_as_insight_list` (`per_task.py:284-340`). `oma.distill.md:52-53`
  requires these MECHANICAL. M7 parser must ADD these checks; adapt `Evidence`
  (swarms `post_id:int`+`task_id` → oma `post_id` = packet-id string, no
  `task_id`; `allowed_post_ids` → cited packet-id set).
  Dated C3 Decision-log entry appended to BOTH `oma.distill.md` copies
  (source-of-truth + repo `docs/design/`), byte-identical. Status: **M7 done**
  (C3-ADD #1/#2 mechanical in `oma.distill.parse`; `Evidence` remapped to
  packet-id strings; anti-meta enforcement shared as code with M6 via
  `oma.forum.anti_meta`; idempotency-key = scope+goal+posts only, logged).
- **C4 — Doc citations (non-blocking).** Cite Design Principles §11 (structure
  ...
  `oma.distill.server`/`resolve` docstrings); **M8 done** (§11 cited in the
  `oma.cli` module docstring + help epilog: agent generates post + proposes ★,
  human one-taps).
...
`integration`/`online` suites opt-in via `OMA_RUN_INTEGRATION=1`/`OMA_RUN_ONLINE=1`.

## Milestone status

- M0 scaffolding — ✅ green (51 files, 15 tests, mkdocs strict, compose valid)
- M1 oma.utils — ✅ green (config/sid/provider/log; 52 tests). httpx added as
  runtime dep. Reconciliation noted: `rate_limit_signal` ports the two
  datasmith-proven Codex/Claude schemas (oma.utils.md Verification requires
...
- M2 oma.bank — ✅ green. 7 append-only migrations 00001..00007; async Bank
  ...
  Decisions: (a) ruff `A002` disabled project-wide — oma's public vocabulary
  (`id`,`type`) mirrors the frozen wire schema + oma.core.md signatures;
  A001/A003+ stay active. (b) Bank API is **async** (locks the seam for M3
  `.fetch()` + M9 FastAPI). (c) Supabase re-ported to **544xx**
  (config.toml) so oma's stack coexists with the sibling datasmith
  Supabase on the same host; `OMA_BANK_URL` default + integration DSN
...
- M3 oma.core — ✅ green. Frozen Pydantic Session/Goal/Agent/Packet +
...
- Post-M3 advisor hardening (4 items, all applied; offline 95 pass/5 skip,
  ...
  per-identity singleton (`_BANKS`); `oma.core` `_PACKET_CACHE` re-keyed
  ...
  `oma.core.models.get_bank`→exploding tripwire; bare ctor + derived props
  silent, `fetch()` trips it.
- M4 oma.capture — ✅ green. `CanonicalTrace`/`TraceEvent`/`ScrubReport`
  ...
  deployed-code constant, NOT an `OMA_*` knob (drives the re-scrub backfill
  ...
  OMA_TRACE_MAX_BYTES`. `persist()` mints `{sid}/{uuid8}` raw packet then
  ...
  Doc-sync: no divergence from frozen oma.capture.md (the doc already frames
...
- M5 oma.adapters — ✅ green. `Adapter` ABC: `name/binary/version/
  ...
  appended to BOTH oma.adapters.md copies — source + repo). Subprocess
  ...
  non-interactive, order local(`OMA_ADAPTERS_DIR/<name>/__init__.py` exporting
  ...
  builtin sample fixture → `oma.capture.validate()` + fidelity asserted.
  `capture()` is RAW (un-scrubbed, proven) — scrub is oma.capture's central
...
- M6 oma.forum — ✅ green. `ANTI_META_BLOCK` ported **byte-identical** from
  swarms `concreteness.py:20-51` into `oma.forum.anti_meta` (single object;
  M7 `oma.distill` imports the *same* one — `is`, not `==`); +
  ...
  tightened (preference distill-only — Decision-log on oma.core.md, both
  ...
  swarms→oma Evidence remap (`evidence_post_id:int`→`evidence_ref:str`, no
  task) — M6 analog of C3; recorded in oma.forum.md Decision log (both
  ...
  accessible via `oma.forum.*`). 156 collected, 151 offline pass / 5 gated
  skip; 17 new forum tests + 1 C1 core test.
- **M7 — oma.distill (curator) — done.** `schema.py` = 6-bucket constants
  ...
  **same `ANTI_META_BLOCK` object imported from oma.forum** + schema, per
  ...
  **string** (no task_id) resolved vs real clustered ids; shared anti-meta
  CODE with M6 (`oma.forum.anti_meta.CONCRETE_RE/is_concrete/has_banned_meta`
  ...
  fallback; `_OpenAICompatModel` `OMA_LLM_*` fallback. `curator.py` =
  ...
  corollary Decision-log appended to BOTH oma.distill.md copies
  (byte-identical). No new runtime deps (httpx lazy, already M2). No
  `_LAZY_IMPORTS` additions (curator is verb-driven; `oma.distill.*`).
  Post-advisor refinements: (a) `OMA_REUSE_WEIGHT` wired into
  `weighting.py` (was a dead OMA_ knob — the one a user tunes for the
  ...
  overwriting violates `oma.bank.md:92` append-only; consumer/`/inject`
  gate decides exclusion — documented in `curator.py` + both
  `oma.distill.md` copies + a dedicated test); (c) idempotency test now
  asserts `again.bundle == pkt.bundle` (not just id).
  182 offline pass / 5 gated skip; 31 new distill tests, 0 M6 regressions.
- **M8 — oma.cli + preflight — done.** Single `oma.cli:main`, dumb
  ...
  (noninteractive⇒deny-by-default), `_oma_home`/active-file via `OMA_HOME`
  (tests never touch real `~/.oma`). Two-stage SIGINT lifted from datasmith
  (`_sigint_count`, lazy `oma.adapters.terminate_all_agents`, SIGTERM→
  SIGKILL+`os._exit`), installed in `main()` not at import. argparse +
  `RawDescriptionHelpFormatter`; `/verb` canonicalized before dispatch;
  unknown first token → `oma <name> [args]`. Async handlers
  ...
  `record_injection`, noninteractive⇒deny. `oma end` ★ → most recent
  ...
  `oma.forum.render_post_prompt` (`forum/prompt.py`, embeds the SAME
  ...
  4 settled CLI-seam decisions: dated entry on BOTH `oma.cli.md` copies
  (byte-identical). **218 offline pass / 5 gated skip; ~36 new tests
  (test_cli + test_preflight + 2 forum), 0 prior-milestone regressions.**
- **M9 — `oma.web` (read-only API + static viewer) — DONE & GREEN.**
  `oma.web.api.create_app(*, bank=…, identity="public")`: identity fixed at
  ...
  pagination reuses `oma.bank.make_cursor`; tested stable across a mid-scan
  insert (no skip / no dup). `oma.web.server.serve()` (uvicorn, mounts
  ...
  `oma.web.md` copies). Runtime deps **`fastapi>=0.110`/`uvicorn>=0.29`
  added once** (both imported in `src/` so deptry DEP002 stays green);
  `uv lock` + `uv lock --locked` pass. New tunables
  `OMA_WEB_PAGE_LIMIT`/`OMA_WEB_MAX_PAGE_LIMIT`/`OMA_WEB_HOST`/
  `OMA_WEB_PORT` (oma.utils) — **add to the M10 env-doc TODO** above.
  `Agent` gained optional `start_date`/`end_date` + `from_activity` builder
  (faithful to `oma.core.md:103`; dated entry on BOTH `oma.core.md` copies).
  Dated M9 entries on BOTH `oma.web.md` copies (byte-identical). No
...
- **M10 — Integration, CI, docs — DONE & GREEN (build complete).**
  Closes the build. (1) **Env-doc TODO discharged**: `oma.env.example` now
  documents every post-M1 tunable — `OMA_ADAPTERS_DIR` (M5), `OMA_HOME` (M8),
  `OMA_WEB_PAGE_LIMIT`/`OMA_WEB_MAX_PAGE_LIMIT`/`OMA_WEB_HOST`/`OMA_WEB_PORT`
  (M9). (2) **C2 closed**: dated entry on BOTH `oma.distill.md` copies names
  ...
  `/inject` human gate); the future-heuristic seam is `oma.distill` →
  ...
  `oma.distill.resolve._discover_local_model` (the package re-export shadows
  ...
  *resolved*, not carried, via a dated M10 entry on BOTH `oma.web.md`
  ...
  in). Doc-sync byte-identical across all three pairs (`oma.distill.md`,
  `oma.web.md`, `oma.core.md`). **237 offline pass / 7 gated skip (+3:
...
doc-synced; the design docs (`Oh-My-Agent/components/oma.*.md`) and their
...
from real use: `oma start` with no Bank key dumped a `RuntimeError` traceback
from `supabase_bank._client()`. Added `oma.cli._guard`: `main()` runs every
verb coroutine through it; an operational failure becomes one actionable line
(cause + "run `python -m oma.preflight`; set `OMA_BANK_TRUSTED_KEY` or
...
`SystemExit` (argparse) re-raised untouched; `OMA_DEBUG=1` re-raises full
traceback (added to `oma.env.example`). Dumb-CLI invariant intact (formats the
failure, owns no logic, points at `preflight`). Dated entry on BOTH
`oma.cli.md` copies (byte-identical). **242 offline pass / 7 gated skip (+5
...
as bash subcommands (`oma /self-distill ...`). The Overview always had them
as in-agent slash commands (`(oma-claude-001) $ /self-distill`); the bash
...
installed by `oma <name>`. The bash CLI keeps only session lifecycle.

**M11.1 substrate — DONE & GREEN.** `oma._mcp` (FastMCP, stdio transport),
...
`$OMA_SESSION` env → `~/.oma/active` fallback. `setup_environment()`
extended to load `$OMA_HOME/env` before `./oma.env`. New runtime deps:
`mcp>=1.0`, `tomlkit>=0.13`. **18 tests in `tests/test_mcp.py`**.

**M11.2 Claude installer + transparency core — DONE & GREEN; 4 defects
real-`~/.claude/`-surfaced and fixed.** Built `oma._installer` first
...
the original code wrote `~/.claude/skills/oma-<verb>/SKILL.md` + merged
`~/.claude/settings.json` `mcpServers.oma`. User smoke surfaced (a)
the SKILL dir name **is** the slash command (`/oma-self-distill` instead
...
--scope user oma -- <python> -m oma._mcp` via `CLIAction`; rewrote
`_pty_spawn` to `pty.fork` + `TIOCSWINSZ` + `SIGWINCH`; post-uninstall
hint added. Real-`~/.claude/` re-verified: `claude mcp list` shows
`oma: ... ✓ Connected` after install, gone after `oma uninstall claude`.
...
under `$OMA_HOME/extensions/gemini-oma/` (oma-owned) and gemini symlinks
...
control). Codex skills are `$oma-<verb>` (Codex reserves `/`). Uninstall
...
`oma._handlers` with **kwargs signatures** (no argparse Namespace
coupling). `oma --help` now shows only the 5 lifecycle verbs. New
...
rewritten to call `oma._handlers.do_*` directly. **Sim narrative
...
the "What `oma <name>` writes to your filesystem" transparency table
(every absolute path per adapter, CREATE-vs-MERGE, reversal). Dated M11
entries on BOTH `oma.cli.md` and `oma.adapters.md` copies
...
- **V2 (in-agent flow)** ✅ — typed `/self-distill` inside a real Claude Code
  spawned by `oma claude --dangerously-skip-permissions`. Claude (a) saw
  the bare `/self-distill` slash (skill dir name → slash, not the YAML
  `name:`), (b) invoked the `oma` MCP tool ("Called oma" marker visible),
...
The M11.3 first pass dropped the formal `OMA_SESSION` env-passthrough
...
smoke: install → `✓ oma (0.1.0) ... Settings: session: [not set]` ↔
