...
Validates that ``oma.adapters.skills.<name>.install`` (and the
...
from oma._installer import load_manifest, uninstall
from oma.adapters.builtin.claude import ClaudeAdapter
from oma.adapters.skills.claude import build_plan, install


@pytest.fixture
def fake_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    """Redirect ``Path.home()`` to a tmp dir so the user's real ~/.claude is
    NEVER touched. Sets ``OMA_INSTALL_SKILLS=auto`` (silent consent) and
...
def captured_cli(monkeypatch: pytest.MonkeyPatch) -> list[list[str]]:
    ...
    import oma._installer as inst_mod
...
def test_install_invokes_claude_mcp_add_with_right_argv(fake_home: Path, captured_cli: list[list[str]]) -> None:
    ...
    assert add[:7] == ["claude", "mcp", "add", "--scope", "user", "oma", "--"]
    assert sys.executable in add and "oma._mcp" in add
...
def test_uninstall_removes_skills_and_runs_claude_mcp_remove(fake_home: Path, captured_cli: list[list[str]]) -> None:
    ...
    assert any(
        argv[:5] == ["claude", "mcp", "remove", "--scope", "user"] and argv[5] == "oma" for argv in real_invocations
...
def test_uninstall_when_claude_binary_absent_records_skip(fake_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    line rather than crashing — the user can run ``claude mcp remove oma``
    manually."""
    import oma._installer as inst_mod
...
# --------------------------------------------------------------------------- #
# Gemini installer — extension bundle staged in $OMA_HOME, registered via CLI
# --------------------------------------------------------------------------- #


def test_gemini_plan_stages_bundle_under_oma_home_with_link_cli(fake_home: Path, captured_cli: list[list[str]]) -> None:
    """Lesson from M11.2: don't file-poke ``~/.gemini/extensions/oma/``
    directly. Source-of-truth bundle lives under ``$OMA_HOME/extensions/
    gemini-oma/``; ``gemini extensions install --skip-settings --consent`` registers it (M11 P1: switched from `link` so the formal env-passthrough allowlist survives — `link` had no settings-skip flag) (was symlink under `link`, now a copy under `install`; pip
    upgrade auto-propagates the bundle, no re-install)."""
    from oma.adapters.skills.gemini import build_plan

    oma_home = fake_home / ".oma"
    plan = build_plan(session_id="S1", oma_home=oma_home, scope="user")
    bundle_root = oma_home / "extensions" / "gemini-oma"
    # 6 CREATE ops write to the oma-owned staging dir; 1 MERGE op pre-trusts
    ...
    assert pre_clear.install_argv == ("gemini", "extensions", "uninstall", "oma")
    ...
    assert register.uninstall_argv == ("gemini", "extensions", "uninstall", "oma")
...
def test_gemini_install_writes_bundle_and_invokes_extensions_install(
    ...
    from oma.adapters.skills.gemini import install
    ...
    # NEVER write ~/.gemini/extensions/oma/ directly.
    assert not (fake_home / ".gemini" / "extensions" / "oma").exists()
...
def test_gemini_adapter_install_skills_delegates(fake_home: Path, captured_cli: list[list[str]]) -> None:
    ...
    m = adapter.install_skills(session_id="S1", oma_home=oma_home, scope="user")
    assert m is not None
    assert (oma_home / "extensions" / "gemini-oma" / "gemini-extension.json").is_file()
...
def test_gemini_uninstall_runs_extensions_uninstall(fake_home: Path, captured_cli: list[list[str]]) -> None:
    ...
    assert rc == 0
    assert any(argv == ["gemini", "extensions", "uninstall", "oma"] for argv in captured_cli)
...
def test_codex_plan_creates_oma_prefixed_skills_and_toml_merges(fake_home: Path) -> None:
    """Codex reserves the ``/`` namespace — skills are invoked as ``$oma-<verb>``."""
    from oma.adapters.skills.codex import build_plan
    ...
    assert sections == {
        "mcp_servers.oma",
        "mcp_servers.oma.tools.commit_post",
        "mcp_servers.oma.tools.inject_commit",
    }


def test_codex_install_writes_skills_and_merges_toml(fake_home: Path) -> None:
    from oma.adapters.skills.codex import install
    ...
    assert "[mcp_servers.oma]" in text
    assert "[mcp_servers.oma.tools.commit_post]" in text
    assert 'approval_mode = "prompt"' in text
    assert "OMA_SESSION" in text  # env_vars allowlist
    for verb in ("self-distill", "discuss", "cross-distill", "inject"):
        assert (fake_home / ".codex" / "skills" / f"oma-{verb}" / "SKILL.md").is_file()
...
def test_codex_install_preserves_third_party_mcp_server_round_trip(fake_home: Path) -> None:
    ...
    ``~/.codex/config.toml``. Install adds ``[mcp_servers.oma]``, uninstall
    ...
    assert "[mcp_servers.oma]" in mid  # our entry landed
    ...
    # After uninstall: oma sections gone, the user's content + comments intact.
    assert after is not None
    assert "[mcp_servers.oma]" not in after
