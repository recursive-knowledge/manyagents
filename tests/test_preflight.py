"""M8/M10 tests for oms.preflight — env / Bank-reachability /
migration-inventory + the M10 best-effort live ``schema_migrations`` diff
(oms.cli.md "preflight.py"). httpx is mocked with respx (the M2 pattern); the
...
def test_env_missing_all_keys_fails(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMS_BANK_URL", "http://127.0.0.1:54421")
...
def test_migrations_present() -> None:
    # The repo ships supabase/migrations/00001..00007 (oms.bank, M2).
...
def test_run_preflight_green_when_env_and_bank_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMS_BANK_URL", "http://127.0.0.1:54421")
    monkeypatch.setenv("OMS_BANK_ANON_KEY", "anon-key")
    monkeypatch.delenv("OMS_BANK_ADMIN_KEY", raising=False)  # no privileged key → live diff skips
