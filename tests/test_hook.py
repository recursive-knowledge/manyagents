"""Tests for ``manyagent._hook`` — the harness lifecycle hook sink (M12 groundwork).
...
``MANYAGENT_SESSION`` it writes nothing and exits 0, (2) for wrapped runs it
appends one JSONL binding record per hook event under
``$MANYAGENT_HOME/bindings/``, and (3) it never raises and never exits nonzero —
...
from manyagent import _hook


@pytest.fixture
def manyagent_home(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    home = tmp_path / ".manyagent"
    monkeypatch.setenv("MANYAGENT_HOME", str(home))
    return home
...
def test_no_manyagent_session_writes_nothing(manyagent_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.delenv("MANYAGENT_SESSION", raising=False)
    assert _hook.main(io.StringIO(_payload())) == 0
    assert not (manyagent_home / "bindings").exists()


def test_appends_one_record_per_event(manyagent_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """One wrapped PTY run can span several harness sessions (`/clear` rolls
    a fresh id) — the sink appends, never overwrites."""
    monkeypatch.setenv("MANYAGENT_SESSION", "trial")
    assert _hook.main(io.StringIO(_payload())) == 0
    assert _hook.main(io.StringIO(_payload(hook_event_name="SessionEnd", reason="clear"))) == 0
    lines = (manyagent_home / "bindings" / "trial.jsonl").read_text().splitlines()
    assert len(lines) == 2
    start, end = (json.loads(line) for line in lines)
    assert start["manyagent_session"] == "trial"
...
def test_garbage_stdin_is_swallowed(manyagent_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MANYAGENT_SESSION", "trial")
    assert _hook.main(io.StringIO("not json {")) == 0
    assert not (manyagent_home / "bindings").exists()


def test_non_object_payload_ignored(manyagent_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("MANYAGENT_SESSION", "trial")
    assert _hook.main(io.StringIO('["a", "list"]')) == 0
    assert not (manyagent_home / "bindings").exists()


def test_slash_in_session_id_refused(manyagent_home: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    """A '/' would escape the bindings dir (and violates the Bank's
    no-slash session-id constraint) — refuse to write anything."""
    monkeypatch.setenv("MANYAGENT_SESSION", "evil/../escape")
    assert _hook.main(io.StringIO(_payload())) == 0
    assert not (manyagent_home / "bindings").exists()
...
def test_session_start_delivers_stashed_injection(
    ...
    monkeypatch.setenv("MANYAGENT_SESSION", "S-1")
    _stash_injection(manyagent_home, "S-1")
    assert _hook.main(io.StringIO(_payload())) == 0
    ...
    assert (manyagent_home / "inject" / "S-1.json").is_file()
