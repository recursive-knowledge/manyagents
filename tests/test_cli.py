"""M8 tests for oma.cli — the dumb orchestrator (oma.cli.md Verification).
...
def _tmp_home(tmp_path: Any, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    # M11: `_do_run_agent` exports OMA_SESSION before PTY spawn; delenv at
    # setup records "was absent" so monkeypatch restores absence at teardown
    # (prevents OMA_SESSION pollution between tests).
    monkeypatch.delenv("OMA_SESSION", raising=False)
    monkeypatch.setenv("OMA_INSTALL_SKILLS", "deny")  # tests never write to ~/.claude
...
def test_argparse_lifecycle_verbs_only() -> None:
    ...
    they're installed as in-agent skills + MCP tools by ``oma <name>``.
    Programmatic callers use ``oma._handlers.do_*`` directly."""
...
async def _boom_runtime() -> int:
    raise RuntimeError("Bank identity 'trusted' has no key (OMA_BANK_TRUSTED_KEY unset)")


def test_guard_translates_bank_error_no_traceback(
    capsys: pytest.CaptureFixture[str], monkeypatch: pytest.MonkeyPatch
) -> None:
    monkeypatch.delenv("OMA_DEBUG", raising=False)
    rc = cli._guard(_boom_runtime())
    assert rc == 1
    err = capsys.readouterr().err
    assert "no key" in err  # the real cause is shown
    assert "python -m oma.preflight" in err and "OMA_DEBUG=1" in err  # actionable


def test_guard_debug_env_reraises(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMA_DEBUG", "1")  # developer escape hatch
...
async def test_main_missing_bank_key_returns_1_not_traceback(
    ...
    """The reported scenario: `oma start` with no Bank key must fail cleanly."""
    ...
    assert "python -m oma.preflight" in capsys.readouterr().err
...
def test_session_url_uses_web_host_and_port(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMA_WEB_HOST", "127.0.0.1")
    monkeypatch.setenv("OMA_WEB_PORT", "9001")
...
def test_session_url_maps_wildcard_bind_to_loopback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv("OMA_WEB_HOST", "0.0.0.0")  # noqa: S104 — testing the defensive rewrite of the wildcard, not a real bind
    monkeypatch.setenv("OMA_WEB_PORT", "8000")
...
async def test_register_prints_open_link(fake_bank: FakeBank, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("OMA_WEB_HOST", "127.0.0.1")
    monkeypatch.setenv("OMA_WEB_PORT", "8580")
...
# --------------------------------------------------------------------------- #
# oma end — ★ lands on the most recent unrated reflection post
# --------------------------------------------------------------------------- #
...
# --------------------------------------------------------------------------- #
# oma <name> — PTY spawn (stdlib call monkeypatched) + skill install + capture
# --------------------------------------------------------------------------- #
...
async def test_run_agent_spawns_pty_installs_skills_and_captures_raw_packet(
    ...
    `raw` packet via `oma.capture.persist` (closes the M8 deferral). The
