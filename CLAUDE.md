...
make test      # pytest + coverage (offline; MANYAGENT_RUN_INTEGRATION/MANYAGENT_RUN_ONLINE opt-in)
make bank-up / bank-migrate / bank-down / bank-status / bank-reset   # local Bank (npx supabase)
make web-up / web-build / web-dev   # read-only viewer (manyagent.web + web/viewer SvelteKit)
...
MANYAGENT_RUN_INTEGRATION=1 uv run pytest -m integration     # opt in to Bank-backed tests
MANYAGENT_RUN_ONLINE=1      uv run pytest -m online          # opt in to live agent/LLM tests
```
...
- **Schema changes are new numbered migrations** under `supabase/migrations/`,
  never edits to existing ones (manyagent.bank; Principles §3).
- **Tunables are `MANYAGENT_`-prefixed**, read from env at module scope, defaulted in
  code, documented in `manyagent.env.example` (Principles §8; manyagent.utils).
- **No per-instance `__getattr__`** dispatch (Principles §4). Package-level
  PEP-562 lazy loading in `src/manyagent/__init__.py` is the allowed, explicit form.
- **Tests mirror `src/manyagent/` 1:1.** Every feature ships with a test.
- **Doc-sync:** when the build forces a divergence from a frozen design doc,
  append a dated entry to that `docs/design/components/manyagent.*.md` Decision log.

## Architecture

Layered, bottom-up — each layer imports only from layers above it in the list:

`manyagent.utils` (config/sid/provider/log) → `manyagent.bank` (Supabase + RLS, retry
shim) → `manyagent.core` (frozen Pydantic Session/Goal/Agent/Packet, Collection) →
`manyagent.capture` (trace → scrubbed `raw` packet, `CanonicalTrace`) →
`manyagent.adapters` (`Adapter` ABC + builtins claude/codex/gemini; local-adapter
discovery via `MANYAGENT_ADAPTERS_DIR`) → `manyagent.forum` (post discipline,
`ANTI_META_BLOCK`) → `manyagent.distill` (curator, 6-bucket Insight schema,
content-addressed idempotent re-runs, no-carry-forward) → `manyagent.cli` (single
entrypoint `manyagent.cli:main`, two-stage SIGINT, writes `~/.manyagent/active`) →
`manyagent.web` (read-only FastAPI + SvelteKit viewer under `web/viewer/`) →
`manyagent._mcp` (M11 in-agent MCP server for Claude Code / Codex / Gemini).

`manyagent.testing` sits atop the stack (it imports cli/_handlers): the dummy
...
Package-level lazy loading lives in `src/manyagent/__init__.py` (`_SUBMODULES` +
...
Every piece of session data is a **Packet** (`manyagent.core.Packet`), one of three types:
...
- **`post`** — An agent's reflection or reply to another post (manyagent.forum). Has
...
- **`distill`** — Curated insights by the distiller (manyagent.distill). Has `scope`
...
**Web API** (`manyagent.web.api`, M9) is read-only, identity-gated, DB-enforced. The
...
require explicit `?include=raw` and are gated by `MANYAGENT_WEB_PUBLIC_RAW` env
